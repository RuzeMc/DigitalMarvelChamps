...
void FPropertyNode::InitNode(const FPropertyNodeInitParams& InitParams)
{
	...
	else
	{
		...
		// HAZE FIX(LV): Implement EditInlineDefaults
		static const FName Name_EditInlineDefaults("EditInlineDefaults");
		bIsEditInlineNew |= InitParams.bCreateDisableEditOnInstanceNodes && bIsObjectOrInterface && GotReadAddresses && MyProperty->HasMetaData(Name_EditInlineDefaults);
		// END HAZE FIX
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	SetNodeFlags( EPropertyNodeFlags::NoCacheAddress, MyProperty && MyProperty->HasAnyPropertyFlags(CPF_RuntimeGenerated) );
// END HAZE FIX
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	if( CachedReadAddresses.Num() && !CachedReadAddresses.bRequiresCache && !HasNodeFlags(EPropertyNodeFlags::RequiresValidation | EPropertyNodeFlags::NoCacheAddress) )
// END HAZE FIX
	{
...
bool FPropertyNode::GetReadAddress( FReadAddressList& OutAddresses ) const
{
	...
	// @todo PropertyEditor Nodes which require validation cannot be cached
// HAZE FIX(LV): Runtime generated classes (angelscript)
	if( CachedReadAddresses.Num() && !HasNodeFlags(EPropertyNodeFlags::RequiresValidation | EPropertyNodeFlags::NoCacheAddress) )
// END HAZE FIX
	{
		OutAddresses.ReadAddressListData = &CachedReadAddresses;
		return true;
...
struct FPropertyItemComponentCollector
{
	...
	void ProcessProperty( FProperty* Property, uint8* PropertyValueAddress )
	{
		if ( Property != NULL )
		{
			...
			// HAZE FIX(LV): Implement EditInlineDefaults
			bContainsEditInlineNew |= Property->HasMetaData(TEXT("EditInlineDefaults")) && ((Property->PropertyFlags & CPF_EditConst) == 0);
			// END HAZE FIX
