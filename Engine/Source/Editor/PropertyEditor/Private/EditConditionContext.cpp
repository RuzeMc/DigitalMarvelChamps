...
{
// AS FIX (Josh Wood <josh@envarstudio.com>): EFindFirstObjectOptions::Exact blocks UUserDefinedEnum, use EFindFirstObjectOptions::None
	const UEnum* EnumType = UClass::TryFindTypeSlow<UEnum>(EnumTypeName, EFindFirstObjectOptions::None);
// END AS FIX
	if (EnumType == nullptr)
	{
		return TOptional<int64>();
