...
void FKCHandler_CallFunction::CheckIfFunctionIsCallable(UFunction* Function, FKismetFunctionContext& Context, UEdGraphNode* Node)
{
	...
	if (!Function->HasAnyFunctionFlags(FUNC_BlueprintCallable | FUNC_BlueprintEvent) && (Function->GetOuter() != Context.NewClass)) // HAZE FIX (JM) - Allow BlueprintImplementableEvent functions to be called from other blueprints
	{
