...
// AS FIX(LV): Custom property list must be updated before PostCDOCompiled, since that can create instances of the object
	...
	if (!TickFunction->bCanEverTick)
	{
		...
		if (const UFunction* ReceiveTickEvent = FKismetCompilerUtilities::FindOverriddenImplementableEvent(ReceiveTickName, NewClass))
		{
			...
			else
			{
				...
				{
// AS FIX(LV): Don't override bCanEverTick if our tick is a script function
					// We're allowed to tick for one reason or another
					if (!ReceiveTickEvent->HasAnyFunctionFlags(FUNC_RuntimeGenerated))
