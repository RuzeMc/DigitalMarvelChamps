	...
	TSharedPtr<SWidget> FSceneOutlinerHelpers::GetClassHyperlink(UObject* InObject)
	{
		if (InObject)
		{
			if (UClass* Class = InObject->GetClass())
			{
				...
				// HAZE FIX(LV): allow script classes here
				if (bIsBlueprintClass || bIsGameClass || Class->bIsScriptClass)
				// END HAZE FIX
				{
					// HAZE FIX(LV): Meta tag for not showing a link
					static const FName NAME_NoSourceLink("NoSourceLink");
					if (Class->HasMetaData(NAME_NoSourceLink))
						return nullptr;
					// END HAZE FIX
