	...
	while (FParse::LineExtended(&SourceText, StrLine, LinesConsumed, true))
	{
		...
		else if( GetBEGIN(&Str,TEXT("Object")))
		{
			...
			else 
			{
				...
				{
					// HAZE FIX(LV): 
					//  Archetype information is not accurate for transient stuff after it's
					//  created, so this archetype check fails even if it is correct.
					//  There's no way to actually fix this, so I'm getting rid of the archetype check.
					//auto* TemplateArchetype = ComponentTemplate->GetArchetype();
					bool bIsOkToReuse = ComponentTemplate->GetClass() == TemplateClass
						...
						//&& (TemplateArchetype == Archetype || !Archetype);
						;
					// END HAZE FIX
