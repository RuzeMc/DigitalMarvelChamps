...
UClass* FBlueprintEditorUtils::FindFirstNativeClass(UClass* Class)
{
	...
	{
// AS FIX (LV): Script classes should count as native classes for blueprint purposes
		if( 0 != (Class->ClassFlags & CLASS_Native) || Class->bIsScriptClass)
// END AS FIX
		{
			break;
		}
...
UClass* FBlueprintEditorUtils::GetMostUpToDateClass(UClass* FromClass)
{
	...
	// HAZE FIX(LV): Runtime generated classes (angelscript)
	if (FromClass->bIsScriptClass)
	{
		return FromClass->GetMostUpToDateClass();
	}
	// END HAZE FIX
...
// HAZE FIX(LV): runtime generated classes (angelscript)
		static void ProcessRuntimeHierarchy(UStruct* Struct, TSet<TWeakObjectPtr<UStruct>>& InDependencies)
		{
...
// HAZE FIX(LV): runtime generated classes (angelscript)
	FGatherDependenciesHelper::ProcessRuntimeHierarchy(InBlueprint->ParentClass, OutUDSDependencies);
// END HAZE FIX
			...
			for (UK2Node* Node : Nodes)
			{
				...
				if (Node && Node->HasExternalDependencies(&LocalDependentStructures))
				{
					for (UStruct* Struct : LocalDependentStructures)
					{
						...
						{
// HAZE FIX(LV): runtime generated classes (angelscript)
							FGatherDependenciesHelper::ProcessRuntimeHierarchy(Struct, OutUDSDependencies);
// END HAZE FIX
...
// HAZE FIX(LV): runtime generated classes (angelscript)
		// Separate out runtime generated UClasses from the UDS list we passed (hacky)
		Blueprint->CachedRuntimeClassDependencies.Empty();
		for (auto& Struct : Blueprint->CachedUDSDependencies)
		{
			UClass* Class = Cast<UClass>(Struct.Get());
			if (Class != nullptr)
				Blueprint->CachedRuntimeClassDependencies.Add(Class);
		}
		for (auto& Class : Blueprint->CachedRuntimeClassDependencies)
			Blueprint->CachedUDSDependencies.Remove(Class);
// END HAZE FIX
...
TSharedRef<SWidget> FBlueprintEditorUtils::ConstructBlueprintParentClassPicker( const TArray< UBlueprint* >& Blueprints, const FOnClassPicked& OnPicked)
{
	...
	if(bIsActor)
	{
		if(bIsLevelScriptActor)
		{
			...
			// HAZE FIX(LV): Allow non-native parent classes
...
void FBlueprintEditorUtils::SanitizeRestrictedContent(UBlueprint* BP)
{
	...
	if (HasRestrictedNodes(BP, &RestrictedNodes))
	{
		...
		{
			RemoveGraph(BP, Graph, EGraphRemoveFlags::MarkTransient);
		}

		// Do a full recompile to bring everything back up-to-date and to sanitize any generated bytecode.
