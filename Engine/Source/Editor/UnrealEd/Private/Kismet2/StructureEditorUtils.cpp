...
// AS FIX (AH): Propagate ScriptStruct changes to editor tools
...
// AS FIX (AH): Propagate ScriptStruct changes to editor tools
FStructureEditorUtils::EStructureEditorChangeInfo FStructureEditorUtils::FScriptStructEditorManager::ActiveChange =
	FStructureEditorUtils::EStructureEditorChangeInfo::Unknown;

FStructureEditorUtils::FScriptStructEditorManager& FStructureEditorUtils::FScriptStructEditorManager::Get()
{
	static TSharedRef<FScriptStructEditorManager> EditorManager(new FScriptStructEditorManager());
	return *EditorManager;
}

void FStructureEditorUtils::BroadcastPreChange(UScriptStruct* Struct)
{
	FStructureEditorUtils::FScriptStructEditorManager::Get().PreChange(Struct, FStructureEditorUtils::FScriptStructEditorManager::ActiveChange);
	TArray<UDataTable*> DataTables = FReinstanceDataTableHelper::GetTablesDependentOnStruct(Struct);
	for (UDataTable* DataTable : DataTables)
	{
		DataTable->CleanBeforeStructChange();
...
void FStructureEditorUtils::BroadcastPostChange(UScriptStruct* Struct)
{
	TArray<UDataTable*> DataTables = FReinstanceDataTableHelper::GetTablesDependentOnStruct(Struct);
	for (UDataTable* DataTable : DataTables)
	{
	...
	FStructureEditorUtils::FScriptStructEditorManager::Get().PostChange(Struct, FStructureEditorUtils::FScriptStructEditorManager::ActiveChange);
}
// END AS FIX
