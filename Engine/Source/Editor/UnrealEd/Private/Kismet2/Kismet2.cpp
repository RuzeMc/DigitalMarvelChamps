...
// HAZE FIX(LV): Allow function meta tags to be default events
			const FName NAME_AutoCreateBPNode("AutoCreateBPNode");
			TArray<UFunction*> DefaultFunctions;
			for (TFieldIterator<UFunction> FuncIter(NewBP->GeneratedClass); FuncIter; ++FuncIter)
			{
				UFunction* Func = *FuncIter;
				if (Func->HasMetaData(NAME_AutoCreateBPNode))
				{
					DefaultFunctions.Add(Func);
				}
			}

			for (int32 i = DefaultFunctions.Num() - 1; i >= 0; --i)
			{
				UFunction* Func = DefaultFunctions[i];
				FKismetEditorUtilities::AddDefaultEventNode(NewBP, NewBP->UbergraphPages[0], Func->GetFName(),
					Cast<UClass>(Func->GetOuter()), NodePositionY);
			}
// END HAZE FIX
		}
