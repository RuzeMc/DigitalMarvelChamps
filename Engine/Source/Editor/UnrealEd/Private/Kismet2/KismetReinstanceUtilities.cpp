...
struct FReplaceReferenceHelper
{
	...
	static void IncludeDSOs(UObject* OldOuter, UObject* NewOuter, TMap<UObject*, UObject*>& OldToNewInstanceMap, TArray<UObject*>& SourceObjects)
	{
		...
		for (UObject* OldSubObj : OldSubObjArray)
		{
			...
			{
				// AS FIX(LV): We call this duplicately sometimes during hotreloadse
				if (!OldToNewInstanceMap.Contains(OldSubObj))
				{
...
// HAZE FIX(LV): Set script type ptr after duplication (angelscript)
	CopyOfOwnerClass->ScriptTypePtr = OwnerClass->ScriptTypePtr;
// END HAZE FIX
...
// HAZE FIX(LV)
	...
	if (NewArchetype == nullptr || OldArchetype == OldClass->GetDefaultObject())
	{
...
// AS FIX(LV): References to this component on the actor need to be replaced immediately,
//             this is because the actor itself might be getting reinstanced _as well_, in which
//             case we want to reinstance it with the *new* component, not with the *old* component!
			TMap<UObject*, UObject*> ComponentReplacementMap;
			ComponentReplacementMap.Add(OldObject, Component);
			FArchiveReplaceObjectRef<UObject> ReplaceComponentRefInActor(
				OwningActor, ComponentReplacementMap,
...
// END AS FIX
		...
		if (bWasRegistered)
		{
			...
			{
				// AS FIX(LV): If the component belongs to an actor that is about to be replaced, don't register it
				if (!OwningActor->GetClass()->HasAnyClassFlags(CLASS_NewerVersionExists))
...
static void ReplaceActorHelper(AActor* OldActor, UClass* OldClass, UObject*& NewUObject, UClass* NewClass, TMap<UObject*, UObject*>& OldToNewInstanceMap, const TMap<UClass*, UClass*>& InOldToNewClassMap, TMap<FSoftObjectPath, UObject*>& ReinstancedObjectsWeakReferenceMap, TMap<UObject*, FActorAttachmentData>& ActorAttachmentData, TArray<FActorReplacementHelper>& ReplacementActors, bool bPreserveRootComponent, bool& bSelectionChanged)
{
	...
	// HAZE FIX(LV): Make sure our root actor isn't attached to anything in this actor, or we have a problem
	if (auto* SceneRoot = NewActor->GetRootComponent())
	{
		if (auto* AttachedTo = SceneRoot->GetAttachParent())
		{
			if (AttachedTo->GetOwner() == NewActor)
			{
				FDetachmentTransformRules Rules(EDetachmentRule::KeepRelative, false);
				SceneRoot->DetachFromComponent(Rules);
			}
		}
	}
	// END HAZE FIX
...
// HAZE FIX(LV): Fix attachments not updating on reloads
	NewActor->ResetInheritedComponentAttachment();

	// Instance components that have been copied by CPFUO need to have their attachments updated as well
	for (auto* InstanceComp : NewActor->GetInstanceComponents())
	{
		if (auto* InstanceSceneComp = Cast<USceneComponent>(InstanceComp))
		{
			auto* Attach = InstanceSceneComp->GetAttachParent();
			if (UObject** ReplacedAttach = OldToNewInstanceMap.Find(Attach))
				Attach = Cast<USceneComponent>(*ReplacedAttach);

			if (Attach != nullptr && Attach->GetOwner() != NewActor && InstanceComp->GetOwner() == NewActor)
			{
				auto* NewAttach = Cast<USceneComponent>(NewActor->GetDefaultSubobjectByName(Attach->GetFName()));
				if (NewAttach != nullptr)
				{
					InstanceSceneComp->SetupAttachment(NewAttach, InstanceSceneComp->GetAttachSocketName());
				}
			}
		}
	}
...
// AS FIX(LV): Do reference replacement on structs in the same pass as classes
UNREALED_API TMap<UObject*, UObject*> GAngelscriptAdditionalReplacementObjects;
// END AS FIX
...
// AS FIX(LV): Allow class references to be replaced too
		void OnObjectsReplaced_AllowClasses(const TMap<UObject*, UObject*>& InReplacedObjects)
		{
			for (const TPair<UObject*, UObject*>& Pair : InReplacedObjects)
			{
				UObject** ExistingReplace = ReplacedObjects.Find(Pair.Key);
				if (ExistingReplace == nullptr || *ExistingReplace == nullptr)
					ReplacedObjects.Add(Pair);
			}
		}
// END AS FIX
...
// AS FIX(LV): Allow class references to be replaced too
	if (Params.bReplaceClassReferences)
		OnObjectsReplacedHandle = FCoreUObjectDelegates::OnObjectsReplaced.AddRaw(&ObjectRemappingHelper, &FObjectRemappingHelper::OnObjectsReplaced_AllowClasses);
	else
		OnObjectsReplacedHandle = FCoreUObjectDelegates::OnObjectsReplaced.AddRaw(&ObjectRemappingHelper, &FObjectRemappingHelper::OnObjectsReplaced);
// END AS FIX
...
// AS FIX(LV): Store actor attachment data _before_ replacing, so we don't break attachments between different actor types
...
// AS FIX(LV): Replace references to the class even if replacing multiple classes
	if(Params.bReplaceClassReferences)
	{
		for (TPair<UClass*, UClass*> OldToNewClass : InOldToNewClassMap)
		{
			...
			FReplaceReferenceHelper::IncludeCDO(OldClass, NewClass, OldToNewInstanceMap, SourceObjects, nullptr);
			FReplaceReferenceHelper::IncludeClass(OldClass, NewClass, OldToNewInstanceMap, SourceObjects, ObjectsReplaced);
		}

		for (TPair<UObject*, UObject*> AdditionalReplace : GAngelscriptAdditionalReplacementObjects)
		{
			OldToNewInstanceMap.Add(AdditionalReplace.Key, AdditionalReplace.Value);
			ObjectRemappingHelper.ReplacedObjects.Add(AdditionalReplace.Key, AdditionalReplace.Value);
			SourceObjects.Add(AdditionalReplace.Key);
		}
	}
// END AS FIX
...
// AS FIX(LV): References to other actors within the actors we're reconstructing should already be replaced now
// This is basically the same as what Epic's wall of text comment says right below, but I think the comment is outdated,
// because there is no actual FArchiveReplaceObjectRef happening anywhere before we call ReplacementActor.Finalize()
//
// I see no way of doing this that doesn't involve running FindAndReplaceReferences twice, unfortunately.
// Construction scripts require old references to already be replaced, but will also add new references to replace to the list.
//
...
// AS FIX(LV): Don't unbind the replace handler until we're done finalizing actors, otherwise we won't detect component replacements correctly
...
// AS FIX(LV): Don't init the animation if we're doing a script reload, because the blueprint hasn't been recompiled yet!
						if (!Params.bReplaceClassReferences)
							SkelComponent->InitAnim(true);
// END AS FIX
	...
	for (AActor* ActorInstance : OwnersToRerunConstructionScript)
	{
		...
		{
			if(ActorInstance->GetWorld()) // HAZEFIX, The sequencer can have objects that is not in world... / Tyko
...
void FBlueprintCompileReinstancer::CopyPropertiesForUnrelatedObjects(UObject* OldObject, UObject* NewObject, bool bClearExternalReferences, bool bForceDeltaSerialization /* = false */, bool bOnlyHandleDirectSubObjects/* = false */, TMap<UObject*, UObject*>* OldToNewInstanceMap /*=nullptr*/, const TMap<UClass*,UClass*>* OldToNewClassMap /*=nullptr*/)
{
	...
	// HAZE FIX(LV): Do delta copy for this always
