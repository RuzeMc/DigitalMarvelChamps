...
// AS FIX (LV): Script classes use their asset type as well
	if (auto* MetaClass = Cast<UClass>(AssetClass))
	{
		auto* ClassAsset = FindObject<UClass>(nullptr, *InAssetData.GetObjectPathString());
		if (ClassAsset != nullptr && ClassAsset->bIsScriptClass)
		{
			if (bOutIsClassType)
			{
				*bOutIsClassType = true;
