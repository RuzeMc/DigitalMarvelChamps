...
// AS FIX (LV): Replacing actors also copies properties for script classes
		else if (bInCopySourceProperties && NewActor != nullptr && NewActor->GetClass()->bIsScriptClass)
		{
			if (NewActor->GetClass()->IsChildOf(OldActor->GetClass()))
			{
				NewActor->UnregisterAllComponents();
				UEngine::FCopyPropertiesForUnrelatedObjectsParams Options;
				Options.bNotifyObjectReplacement = true;
