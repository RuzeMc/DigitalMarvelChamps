...
bool UEditorUtilitySubsystem::TryRun(UObject* Asset)
{
	...
	// AS FIX (PG): Allow running pure AS Blutility classes
	else if (UClass* MaybeScriptClass = Cast<UClass>(Asset))
	{
		if (MaybeScriptClass->bIsScriptClass)
		{
			ObjectClass = MaybeScriptClass;
		}
	}
	// END AS FIX
