...
// AS FIX(LV): Allow script properties to specify display names as well
		const FText PropertyDesc = (Property->IsNative() || Property->HasAnyPropertyFlags(CPF_RuntimeGenerated)) ? Property->GetDisplayNameText() : FText::FromName(PropertyName);
// END AS FIX
...
// HAZE FIX(LV): Allow hiding events here
			 && !Function->HasMetaData("HideEventInBP")
// END HAZE FIX
