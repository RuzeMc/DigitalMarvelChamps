...
// HAZE FIX(LV): Obey SkipReinstancing here
...
// HAZE FIX(LV): Copy default properties from old CDO earlier

					// Before, unreal would only copy properties from the old CDO during the reinstance step
					// and not use a delta copy there, causing all properties to be copied.

					// Currently, for angelscript reasons, we need to do a delta copy in the reinstance step,
					// or property changes in angelscript cdos will not propagate to child classes.

					// Therefore we need to fill our cdo here already, because it may be used as a template by subclasses in
					// later iterations of this loop, and we need to have the correct values in that template,
					// or the delta CPFUO later will break.

					FKismetCompilerContext& CompilerContext = *(CompilerData.Compiler);
					TObjectPtr<UObject>* FoundOldCDO = OldCDOs.Find(BP);
					if (FoundOldCDO != nullptr)
						CompilerContext.OldCDO = *FoundOldCDO;
...
// AS FIX(LV): Don't re-link new script classes
		if (ClassToReinstance->bIsScriptClass)
			continue;
// END AS FIX
...
// AS FIX(LV): Properly reparent subclasses of subclasses
			UClass* const* NewParent = OldClassToNewClassIncludingChildren.Find(ClassToReinstance->GetSuperClass());
// END AS FIX
			if(NewParent)
			{
...
// AS FIX(LV): Allow replacing class references during ReparentHierarchies
	bool bAllClassesAreScript = true;
	for (const TPair<UClass*, UClass*>& OldToNewClass : OldToNewClasses)
	{
		if (!OldToNewClass.Key->bIsScriptClass)
			bAllClassesAreScript = false;
	}

	if (bAllClassesAreScript)
		BatchOptions.bReplaceClassReferences = true;
// END AS FIX
...
// AS FIX(LV): UpdateBytecodeReferences recreates CDOs, so it needs to be done *after* PropagateValuesToCDO for any parent classes
		// -- removed, code put below
// END AS FIX
	}
...
// AS FIX(LV): Destroy the unversioned schema for classes that are being reinstanced, so we don't end up using old data later and crashing
	for (const FReinstancingJob* ReinstancingJobPtr : ReinstancersPtr)
	{
		TSharedPtr<FBlueprintCompileReinstancer> const& Reinstancer = ReinstancingJobPtr->Reinstancer;
		if (!Reinstancer.IsValid())
		...
		if (NewClass != nullptr)
		{
			extern COREUOBJECT_API void DestroyUnversionedSchema(const UStruct* Struct);
			DestroyUnversionedSchema(NewClass);
		}
	}
// END AS FIX
...
// AS FIX(LV): UpdateBytecodeReferences recreates CDOs, so it needs to be done *after* PropagateValuesToCDO for any parent classes
						if ((ThisExport.ObjectFlags & RF_ClassDefaultObject) && (ThisExport.ObjectName == OldCDO->GetFName()))
// END AS FIX
						{
							OldCDOIndex = i;
							break;
...
// AS FIX: GetObjectsOfClass filters by ClassDefaultObject by default, this was mistakingly changed to NoFlags in CL 34548253
			GetObjectsOfClass(OldClass, ArchetypeObjects, /*bIncludeDerivedClasses*/false, RF_ClassDefaultObject, EInternalObjectFlags::Garbage);
// END AS FIX
...
{
// AS FIX (LV): Properly delay oncompiled broadcast until after reinstance here
	if(BPCMImpl)
	{
		// We suppress normal compilation broadcasts because the old code path 
		// did this after GC and we want to match the old behavior:
		...
		TArray<UBlueprint*> CompiledBlueprints;
		TArray<UBlueprint*> SkeletonCompiledBlueprints;
		BPCMImpl->FlushCompilationQueueImpl(bSuppressBroadcastCompiled, &CompiledBlueprints, &SkeletonCompiledBlueprints, nullptr);
		BPCMImpl->FlushReinstancingQueueImpl();
