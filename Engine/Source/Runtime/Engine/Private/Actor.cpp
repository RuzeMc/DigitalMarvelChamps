...
#if WITH_EDITOR
	ResetInheritedComponentAttachment();
#endif
// END HAZE FIX
}

// HAZE FIX(LV): Fix attachments not updating on reloads
void AActor::ResetInheritedComponentAttachment()
{
#if WITH_EDITOR
	...
	{
		AActor* Actor;
		TSet<USceneComponent*> FixedComponents;

		void FixComponentAttachment(USceneComponent* SceneComp)
		{
			if (FixedComponents.Contains(SceneComp))
				return;

			FixedComponents.Add(SceneComp);

			if (!SceneComp->IsDefaultSubobject())
				return;

			auto* Archetype = Cast<USceneComponent>(SceneComp->GetArchetype());
			if (Archetype == nullptr)
				return;
			if (Archetype->HasAnyFlags(RF_ClassDefaultObject))
				return;
			if (Archetype->GetOuter() == Actor)
				return;
			if (!Actor->GetClass()->IsChildOf(Archetype->GetOuter()->GetClass()))
				return;

			auto* ArchetypeAttach = Archetype->GetAttachParent();
			if (ArchetypeAttach != nullptr)
			{
				if (!ensure(ArchetypeAttach->IsTemplate()))
					return;
				if (!ensure(ArchetypeAttach->GetOuter() == Archetype->GetOuter()))
					return;
				if (!ensure(ArchetypeAttach->IsDefaultSubobject()))
					return;
			}

			USceneComponent* InstanceOfArchetypeAttach = nullptr;
			if (ArchetypeAttach != nullptr)
			{
				InstanceOfArchetypeAttach = FindObjectFast<USceneComponent>(Actor, ArchetypeAttach->GetFName());
				check(InstanceOfArchetypeAttach != nullptr);
			}

			if (InstanceOfArchetypeAttach != SceneComp->GetAttachParent())
			{
				// De-attach ourselves
				if (SceneComp->IsRegistered() || SceneComp->GetAttachParent() != nullptr || InstanceOfArchetypeAttach->GetAttachChildren().Num() != 0)
				{
					FAttachmentTransformRules Rules(EAttachmentRule::KeepRelative, false);
					SceneComp->AttachToComponent(nullptr, Rules, SceneComp->GetAttachSocketName());
				}
				else
				{
					SceneComp->SetupAttachment(nullptr, SceneComp->GetAttachSocketName());
				}

				// Fix component attachment recursively upwards
				USceneComponent* TargetAttach = InstanceOfArchetypeAttach;
				while (TargetAttach != nullptr)
				{
					FixComponentAttachment(TargetAttach);
					TargetAttach = TargetAttach->GetAttachParent();
				}

				// Re-attach to the correct one here
				if (SceneComp->IsRegistered() || SceneComp->GetAttachParent() != nullptr || InstanceOfArchetypeAttach->GetAttachChildren().Num() != 0)
				{
					FAttachmentTransformRules Rules(EAttachmentRule::KeepRelative, false);
					SceneComp->AttachToComponent(InstanceOfArchetypeAttach, Rules, SceneComp->GetAttachSocketName());
				}
				else
				{
					SceneComp->SetupAttachment(InstanceOfArchetypeAttach, SceneComp->GetAttachSocketName());
				}
			}
		};
	} AttachmentFixer;

	if (!HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject) && RootComponent != nullptr)
	{
		auto* Archetype = Cast<AActor>(GetArchetype());
		if (Archetype != nullptr)
		{
			auto* ArchetypeRoot = Archetype->GetRootComponent();
			if (ArchetypeRoot != nullptr)
			{
				auto* InstanceOfArchetypeRoot = FindObjectFast<USceneComponent>(this, ArchetypeRoot->GetFName());
				if (InstanceOfArchetypeRoot != nullptr && RootComponent != InstanceOfArchetypeRoot)
					RootComponent = InstanceOfArchetypeRoot;
			}
		}
	}

	// Fix root component attachment if we need to
	...
	// Fix the attachment to match the archetype for all components
	AttachmentFixer.Actor = this;
	for (auto* Comp : GetComponents())
	{
		...
		AttachmentFixer.FixComponentAttachment(SceneComp);
	}

	// Fix all default subobjects - if they have changed their base
	// class the property may have ended up as null and we should fix it
	for (TFieldIterator<FProperty> It(GetClass()); It; ++It)
	{
		FProperty* Prop = *It;
		if (!Prop->HasAllPropertyFlags(CPF_RuntimeGenerated | CPF_InstancedReference))
			continue;
		...
		UObject* Value = ObjProp->GetObjectPropertyValue_InContainer(this);
		if (IsValid(Value))
			continue;
		...
		UObject* FoundComp = FindObjectFast<UObject>(this, Prop->GetFName());
		if (FoundComp == nullptr)
			continue;

		if (!FoundComp->IsA(ObjProp->PropertyClass))
			continue;

		ObjProp->SetObjectPropertyValue_InContainer(this, FoundComp);
	}
#endif
}
// END HAZE FIX
...
// HAZE FIX(LV): Fix attachments not updating on reloads
#if WITH_EDITOR
	// If our root component is different from our archetype's root component we should update that now
	if (Ar.IsLoading() && !(Ar.GetPortFlags() & PPF_Duplicate) && !HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject) && RootComponent != nullptr)
	{
		if (GetClass()->HasAnyClassFlags(CLASS_CompiledFromBlueprint) || GetClass()->bIsScriptClass)
		{
			auto* Archetype = Cast<AActor>(GetArchetype());
...
// AS FIX (LV): Angelscript functions
