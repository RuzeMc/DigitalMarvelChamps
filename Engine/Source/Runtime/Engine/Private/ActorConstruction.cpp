		...
		if (Component)
		{
			...
			if (!bDestroyComponent)
			{
				...
				if (Component->CreationMethod == EComponentCreationMethod::Native && Component->HasAnyFlags(RF_DefaultSubObject))
				{
					...
					{
// AS FIX (LV): During reinstancing, if the component class and the actor class are both being reinstanced at the same time,
//              we may end up with a default component counting as orphaned here. In that case, we do _not_ want to destroy it,
//              because we're about to copy data from it to the new instance of the component, and destroying it would cause
//              the values of all the properties to be lost during the reload.
						if (!GIsReinstancing)
							bDestroyComponent = true;
// END AS FIX
					}
...
// HAZE FIX(LV): Angelscript classes
		// If the root component was constructionscript created,
		// set its transform appropriately now.
		if (RootComponent && RootComponent->CreationMethod == EComponentCreationMethod::UserConstructionScript)
		{
			RootComponent->SetWorldTransform(OldTransform);
		}
// END HAZE FIX
...
// HAZE FIX(LV): Script classes should also run BP construction script data collection
	// If this actor has a blueprint lineage, go ahead and run the construction scripts from least derived to most
	if ((ParentBPClassStack.Num() > 0) || GetClass()->bIsScriptClass)
// END HAZE FIX
	{
		if (bErrorFree)
		{
