		...
		else
		{
			...
			while (ChildCount > 0)
			{
				if (USceneComponent* Child = AttachChildren.Last())
				{
					...
					{
// AS FIX (LV): Hot reloads can cause components to have a null attachment here, in which case DetachFromComponent won't remove it from the AttachChildren list
						...
						else if (ChildAttachParentWas == this)
// END AS FIX
						{
							bool bNeedsDetach = true;
							// If the child is also being destroyed during GC, don't reattach it to anything
...
#if WITH_EDITORONLY_DATA
							// If we are in the middle of a transaction it isn't entirely unexpected that an AttachParent/AttachChildren pairing is wrong
							// AS FIX(LV): Don't complain about wrong attachments for components whose owner is going to be destroyed anyway
							if (!ensureAlwaysMsgf(GIsTransacting || !IsValid(GetOwner()) || GetClass()->HasAnyClassFlags(CLASS_NewerVersionExists), TEXT("Component '%s' has '%s' in its AttachChildren array, however, '%s' believes it is attached to '%s'"), *GetFullName(), *Child->GetFullName(), *Child->GetFullName(), *Child->GetAttachParent()->GetFullName()))
							// END AS FIX
#endif
							{
								// We've gotten in to a bad state where the Child's AttachParent doesn't jive with the AttachChildren array
...
// AS FIX(LV): During angelscript reloads, we swap the attachment to the new instance of the parent, but it will have already copied the child,
//             so we are already in the AttachChildren list. Make sure we don't add duplicates here.
		if (!Parent->AttachChildren.Contains(this))
		{
			...
			{
				Parent->AttachChildren.AddUnique(this);
			}
		}
// END AS FIX
