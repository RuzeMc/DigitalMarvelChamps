...
// HAZE FIX(LV): Input FName instead of FString
FTimerHandle UKismetSystemLibrary::K2_SetTimer(UObject* Object, FName FunctionFName, float Time, bool bLooping, bool bMaxOncePerFrame, float InitialStartDelay, float InitialStartDelayVariance)
{
// END HAZE FIX
	...
	if (Object)
	{
		...
		if ( Func && (Func->ParmsSize > 0) )
		{
			...
			UE_LOG(LogBlueprintUserMessages, Warning, TEXT("SetTimer passed a function (%s) that expects parameters."), *FunctionFName.ToString());
			return FTimerHandle();
		}
	}
