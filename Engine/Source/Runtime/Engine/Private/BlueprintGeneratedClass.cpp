...
// AS FIX (LV): Count angelscript functions as script here as well
...
//HAZE FIX (LV):
				//Don't use objects with non-stable names as archetype objects
				if (Archetype && Archetype->GetFName().GetNumber() != 0)
				{
					Archetype = nullptr;
				}
//END HAZE FIX
...
// AS FIX(LV): Stop at script classes as well
		if (SuperClass->HasAllClassFlags(CLASS_Native) || SuperClass->bIsScriptClass)
// END AS FIX
		{
