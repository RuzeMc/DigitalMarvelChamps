...
public:
	...
	virtual FArchive& operator<<( UObject*& Obj ) override
	{
		if (Obj != NULL)
		{
			...
			{
// HAZE FIX(LV): Null out references if we're replacing something with an instance
// that cannot be placed in this property (base class changed).
				T* ReplaceObject = *ReplaceWith;

				FProperty* Property = GetSerializedProperty();
				if (auto* ObjectProperty = CastField<FObjectPropertyBase>(Property))
				{
					if (ObjectProperty->PropertyClass != nullptr)
					{
						if (ReplaceObject != nullptr && !ReplaceObject->GetClass()->GetMostUpToDateClass()->IsChildOf(ObjectProperty->PropertyClass->GetMostUpToDateClass()))
						{
							if (ObjectProperty->PropertyClass->HasAnyClassFlags(CLASS_Native) || ObjectProperty->PropertyClass->bIsScriptClass)
							{
								ReplaceObject = nullptr;
							}
						}
					}
				}
