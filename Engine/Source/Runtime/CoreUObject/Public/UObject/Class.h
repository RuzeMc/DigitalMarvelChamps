...
// HAZE FIX(LV): runtime generated classes (angelscript)
	virtual int32 GetContainerSize() const
	{
		return PropertiesSize;
	}
// END HAZE FIX
...
// HAZE FIX(LV): Angelscript
		virtual void GetCppTraits(bool& OutHasConstructor, bool& OutHasDestructor, bool& OutHasAssignment, bool& OutHasCopyConstructor) const = 0;
// END HAZE FIX
...
// HAZE FIX(LV): Angelscript
		virtual void GetCppTraits(bool& OutHasConstructor, bool& OutHasDestructor, bool& OutHasAssignmentOperator, bool& OutHasCopyConstructor) const override
		{
			OutHasConstructor        = std::is_default_constructible<CPPSTRUCT>::value && !std::is_trivially_default_constructible<CPPSTRUCT>::value;
			OutHasDestructor         = std::is_destructible<CPPSTRUCT>::value          && !std::is_trivially_destructible<CPPSTRUCT>::value;
			OutHasAssignmentOperator = std::is_copy_assignable<CPPSTRUCT>::value       && !std::is_trivially_copy_assignable<CPPSTRUCT>::value;
			OutHasCopyConstructor    = std::is_copy_constructible<CPPSTRUCT>::value    && !std::is_trivially_copy_constructible<CPPSTRUCT>::value;
		}
// END HAZE FIX
...
// AS FIX: Runtime generated classes (angelscript)
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	COREUOBJECT_API void SetCppStructOps(ICppStructOps* Ops)
	{
		CppStructOps = Ops;
	}

	COREUOBJECT_API virtual UScriptStruct* GetNewestVersion()
	{
		return this;
	}
// END HAZE FIX
...
public:
	...
	virtual void RuntimeConstructObject(UObject* Object) {}
	...
	virtual void GetLifetimeScriptReplicationList(TArray<class FLifetimeProperty>& OutLifetimeProps) const
	{
		...
		if (SuperClass != nullptr)
			SuperClass->GetLifetimeScriptReplicationList(OutLifetimeProps);
	}
	// END HAZE FIX
