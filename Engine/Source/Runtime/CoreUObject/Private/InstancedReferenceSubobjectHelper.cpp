				...
				if (bNewObjectHasOldOuter)
				{
					...
					if (bKeptByOld)
					{
						if (bNotHandledYet)
						{
							...
							{
// AS FIX (LV):
// We need to make sure we clear the loading flags from any subobject that we're going to overwrite
// It's going to get discarded anyway, and the linker will complain otherwise.
								ExistingObject->ClearFlags(RF_NeedLoad | RF_NeedPostLoad);
// END AS FIX
