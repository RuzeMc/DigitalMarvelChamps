...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	// TODO: This can probably be done more efficiently somehow
	auto* Parent = GetClass();
	while (Parent != nullptr)
	{
		if (Parent->bIsScriptClass)
			Parent->RuntimeConstructObject(this);
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	bool bNeedInitialize = !(Class->HasAnyClassFlags(CLASS_Native | CLASS_Intrinsic) || Class->bIsScriptClass);
// END HAZE FIX
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
		bool bIsScriptCDO = Class->bIsScriptClass && Obj->HasAllFlags(RF_ClassDefaultObject);
		if (bIsScriptCDO)
			bCanUsePostConstructLink = true;

		// If we're copying from a script class' default object, we don't need to copy every property,
		// the script class' constructor will set it up to be the same
		bool bSkipInitCustom = false;
		if (DefaultData == DefaultsClass->GetDefaultObject(false) && DefaultsClass->bIsScriptClass)
		{
			bSkipInitCustom = !bCanUsePostConstructLink;
			bCanUsePostConstructLink = true;
		}
// END HAZE FIX
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
		if (bCanUsePostConstructLink && !bSkipInitCustom)
// END HAZE FIX
		{
...
UObject* StaticConstructObject_Internal(const FStaticConstructObjectParameters& Params)
{
	...
	// HAZE FIX(LV): Script objects can be subobjects
	const bool bIsNativeClass = InClass->HasAnyClassFlags(CLASS_Native | CLASS_Intrinsic) || InClass->bIsScriptClass;
	// END HAZE FIX
