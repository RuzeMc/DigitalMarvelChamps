...
// HAZE FIX(LV): Expose type-erased method pointers to native C++ methods
		Class->GenericFuncPtrMap.Add(UTF8_TO_TCHAR(InArray->NameUTF8),
			TPair<FGenericFuncPtr, ASAutoCaller::FunctionCaller>{
				InArray->DirectGenericPtr,
				InArray->Caller
		});
// END HAZE FIX
	}
...
// END HAZE FIX
		bool bOwnedByNativeClass = OwnerClass && (OwnerClass->HasAnyClassFlags(CLASS_Native | CLASS_Intrinsic) || OwnerClass->bIsScriptClass);
// END HAZE FIX
...
// AS FIX(LV): Don't initialize properties on script CDOs from parent
				if (!HasAnyClassFlags(CLASS_Native | CLASS_Intrinsic) && !bIsScriptClass)
// END AS FIX
				{
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	if (NewSuperStruct != nullptr)
		ScriptTypePtr = ((UClass*)NewSuperStruct)->ScriptTypePtr;
// END HAZE FIX
