...
void UObject::CallFunction( FFrame& Stack, RESULT_DECL, UFunction* Function )
{
	...
	// END HAZE FIX
	{
		...
		if (FunctionCallspace & FunctionCallspace::Local)
		{
			...
			else // Runtime (angelscript)
			{
				// Always call RuntimeCallFunction on the class that
				// the function was generated in, not the class of the current
				// object, so the appropriate system receives it.
...
void ProcessLocalFunction(UObject* Context, UFunction* Fn, FFrame& Stack, RESULT_DECL)
{
	...
	auto ContinueProcessLocalFuntionInner = [&]() {
		...
		// HAZE FIX(LV): Angelscript functions
		else if (Fn->FunctionFlags & FUNC_RuntimeGenerated)
		{
			// Always call RuntimeCallFunction on the class that
			// the function was generated in, not the class of the current
			// object, so the appropriate system receives it.
			Fn->RuntimeCallFunction(Context, Stack, RESULT_PARAM);
		}
		// END HAZE FIX
		else
		{
...
void UObject::ProcessEvent( UFunction* Function, void* Parms )
{
	...
	if ((Function->FunctionFlags & (FUNC_Native | FUNC_RuntimeGenerated)) != 0)
	{
	// END HAZE FIX
			...
			if (ValidateFunction)
			{
				...
				for (int32 ParamIdx = 0; ParamIdx < Function->NumParms; ParamIdx++)
				{
						...
						&& ((SourceProp->PropertyFlags & CPF_ReturnParm) == 0))
					{
						...
						const uint8* SrcPtr = SourceProp->ContainerPtrToValuePtr<uint8>(Parms);
				...
				if (*(uint8*)RetPtr != 0)
				{
					Function->RuntimeCallEvent(this, Parms);
				}
				else
				{
