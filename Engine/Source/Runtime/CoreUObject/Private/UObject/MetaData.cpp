...
void UMetaData::CopyMetadata(UObject* SourceObject, UObject* DestObject)
{
	...
	// HAZE FIX (PG/LV): Mapping for DestObject can be created at the line above if they are in the same package
	// so we need to get a new pointer to the map
	DestMetadata->ObjectMetaDataMap.FindOrAdd(DestObject);
	SourceMap = GetMapForObject(SourceObject);
	// END HAZE FIX
