...
bool FBlueprintSupport::IsInBlueprintPackage(UObject* LoadedObj)
{
	...
	{	
		ForEachObjectWithOuterBreakable(Pkg, [&bHasBlueprintClass](UObject* PkgObj)
		{
			...
			{
// AS FIX(LV): Script classes have the compiled from blueprint flag but are not blueprints for the purpose of this check
				bHasBlueprintClass = PkgClass && PkgClass->HasAnyClassFlags(CLASS_CompiledFromBlueprint) && !PkgClass->bIsScriptClass;
// END AS FIX
				return false; // break
			}
...
// AS FIX(LV): Script classes have the compiled from blueprint flag but are not blueprints for the purpose of this check
		if (ClassObject && ClassObject->HasAnyClassFlags(CLASS_CompiledFromBlueprint) && !ClassObject->bIsScriptClass)
 		{
 			return true;
 		}
			...
			&& !TestObject->GetClass()->bIsScriptClass)
 		{
 			return true;
 		}
// END AS FIX
...
// HAZE FIX(LV): Runtime generated classes (angelscript)
	...
	if (Parent->ScriptTypePtr != nullptr)
	{
		while (Parent != nullptr)
		{
			if (Parent->bIsScriptClass)
			{
				Parent->RuntimeDestroyObject(this);
				break;
			}
