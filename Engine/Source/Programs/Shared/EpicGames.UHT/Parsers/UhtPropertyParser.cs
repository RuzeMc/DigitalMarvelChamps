...
// HAZE FIX(LV): Expose type-erased method pointers to native C++ methods
					if (outProperty is UhtObjectPropertyBase)
					{
						outProperty.MetaData.Add("PointerConst", "");
					}
					else
					{
						outProperty.PropertyFlags |= EPropertyFlags.ImplementationConstRef;
					}
// END HAZE FIX
...
// HAZE FIX(LV): Expose type-erased method pointers to native C++ methods
				outProperty.PropertyFlags |= EPropertyFlags.ImplementationNonConstRef;
// END HAZE FIX
			}
...
// HAZE FIX(LV): Expose type-erased method pointers to native C++ methods
					propertySettings.PropertyFlags |= EPropertyFlags.ImplementationConstRef;
// END HAZE FIX
				}
			}
