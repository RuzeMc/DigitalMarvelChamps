						...
						if (function.MetaData.ContainsKey("WorldContext"))
						{
							...
							foreach (UhtType Child in function.Children)
							{
								if (Child is UhtProperty Property)
								{
									...
									{
										Property.PropertyFlags |= EPropertyFlags.WorldContext;
									}
								}
							}
						}
...
// AS FIX (LV): Angelscript support
			EPropertyFlags disallowFlags = ~(EPropertyFlags.ParmFlags | EPropertyFlags.AutoWeak | EPropertyFlags.RepSkip | EPropertyFlags.UObjectWrapper | EPropertyFlags.NativeAccessSpecifiers) & ~EPropertyFlags.NeverDisallow;
// END AS FIX
