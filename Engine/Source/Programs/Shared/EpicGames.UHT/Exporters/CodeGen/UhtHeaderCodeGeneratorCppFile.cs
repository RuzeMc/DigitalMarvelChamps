...
namespace EpicGames.UHT.Exporters.CodeGen
{
	...
	internal class UhtHeaderCodeGeneratorCppFile : UhtHeaderCodeGenerator
	{
		...
		private StringBuilder AppendClassFunctionCallback(StringBuilder builder, UhtClass classObj, UhtFunction function)
		{
			...
			if (!isInterfaceClass)
			{
				builder.Append("static const FName NAME_").Append(classObj.SourceName).Append('_').Append(function.EngineName).Append(" = FName(TEXT(\"").Append(function.EngineName).Append("\"));\r\n");
			}
...
// AS FIX (LV): Angelscript support
		private static string GetOriginalCppDeclaration(UhtProperty? Property)
		{
			...
			if (Property.IsStaticArray)
				return "";
			if (Property is UhtInterfaceProperty)
				return "";

			bool bIsRef = Property.PropertyFlags.HasFlag(EPropertyFlags.ImplementationNonConstRef);
			bool bIsConst = Property.PropertyFlags.HasFlag(EPropertyFlags.ImplementationConstRef);

			StringBuilder builder = new();
			if (bIsConst)
				builder.Append("const ");

			if (Property is UhtByteProperty ByteProp && ByteProp.Enum != null && ByteProp.PropertyFlags.HasFlag(EPropertyFlags.ImplementationEnumAsByte))
			{
			...
			bool bPtrConst = Property.MetaData.ContainsKey("PointerConst");
			if (bPtrConst)
				builder.Append(" const");
			if (bIsRef)
				builder.Append("&");
			...
			{
				{
					...
					if (functions.Instances.All(s => s.FunctionFlags.HasFlag(EFunctionFlags.Static)))
					{
						builder.Append("\tstatic constexpr FNameNativePtrPair Funcs[] = {\r\n");
					}
					else
					{
...
// AS FIX (LV): Angelscript support
						...
						if (classObj.NativeInterface != null)
							bBindMethodPtr = false;
						...
						if (bBindMethodPtr)
						{
							if (function.HasReturnProperty)
							{
								ReturnStr = GetOriginalCppDeclaration(function.ReturnProperty);
						...
						if (bBindMethodPtr)
						{
							...
							foreach (UhtType Child in function.Children)
							{
								if (Child is UhtProperty Property)
								{
									...
									string ParamType = GetOriginalCppDeclaration(Property);
							...
							ParamStr += ")";
							if (function.FunctionExportFlags.HasFlag(UhtFunctionExportFlags.DeclaredConst))
								ParamStr += "const";
						}

						if (bBindMethodPtr)
						{
							string FunctionImpl = function.CppImplName;
							if (function.FunctionFlags.HasFlag(EFunctionFlags.Event))
								FunctionImpl = function.SourceName;

							if (function.FunctionFlags.HasFlag(EFunctionFlags.Static))
							{
								// Static methods should be exposed as function pointers directly
								GenericPtrStr += "ERASE_FUNCTION_PTR(&";
								GenericPtrStr += classObj.SourceName;
								GenericPtrStr += "::";
								GenericPtrStr += FunctionImpl;
								GenericPtrStr += ",";
								GenericPtrStr += ParamStr;
								GenericPtrStr += ",ERASE_ARGUMENT_PACK(";
								GenericPtrStr += ReturnStr;
								GenericPtrStr += "))";
							}
							else
							{
								// Instance methods should be exposed through method type erasure
								GenericPtrStr += "ERASE_METHOD_PTR(";
								GenericPtrStr += classObj.SourceName;
								GenericPtrStr += ",";
								GenericPtrStr += FunctionImpl;
								GenericPtrStr += ",";
						...
						if (!bBindMethodPtr)
							GenericPtrStr = "ERASE_NO_FUNCTION()";
