...
// AS FIX (PG): Allow registering Angelscript Validator Classes on startup, the ModuleName for these is "Angelscript", which doesn't exist
				if (ValidatorClass->bIsScriptClass || FModuleManager::Get().IsModuleLoaded(ModuleName))
// END AS FIX
				{
