...
void FStateTreeNodeClassCache::CacheClasses()
{
	for (TEnumerateRef<FRootClassContainer> RootClass : EnumerateRange(RootClasses))
	{
		...
		if (const UClass* Class = Cast<UClass>(RootClass->BaseStruct))
		{
			...
			{
				UClass* TestClass = *It;
// AS FIX: Allow using angelscript nodes in State Tree
				const bool bValidASNode = TestClass->bIsScriptClass && !TestClass->HasAnyClassFlags(CLASS_NewerVersionExists);
				if ((TestClass->HasAnyClassFlags(CLASS_Native) || bValidASNode) && TestClass->IsChildOf(Class))
// END AS FIX: Allow using angelscript nodes in State Tree
				{
