 ...
 */
struct ANGELSCRIPTCODE_API FAngelscriptType : TSharedFromThis<FAngelscriptType>
{
	/**
	...
	// Whether we can compare values of this type from the engine level during a hot reload
	virtual bool CanCompareForHotReload(const FAngelscriptTypeUsage& Usage) const { return CanCompare(Usage); }

	// Compare whether the values at the source and destination are equal during a hot reload
	virtual bool IsValueEqualForHotReload(const FAngelscriptTypeUsage& Usage, void* SourcePtr, void* DestinationPtr) const { return IsValueEqual(Usage, SourcePtr, DestinationPtr); }
 ...
 */
struct ANGELSCRIPTCODE_API FAngelscriptTypeUsage
{
