...
public:
	...
	UFUNCTION(ScriptCallable, Meta = (ScriptTrivial))
	static FVector GetSafeNormal2D(const FVector& Vector, const FVector& UpDirection, double Tolerance = 0.0000001, const FVector& ResultIfZero = FVector::ZeroVector)
	{
		return FVector::VectorPlaneProject(Vector, UpDirection).GetSafeNormal(Tolerance, ResultIfZero);
	}
...
public:
	 ...
	 * This converts a ChildWorldRotation to a ChildRelativeLocation.
	 *
	 * eg:
	 ...
	 * This converts a ChildRelativeRotation to a ChildWorldLocation.
	 *
	 * eg:
	 * FQuat ChildRelativeRotation = FQuat::GetRelative(ParentWorldRotation, ChildWorldRotation);
	 * FQuat ChildWorldRotation = FQuat::ApplyRelative(ParentWorldRotation, ChildRelativeRotation);
...
public:
	 ...
	 * This converts a ChildWorldTransform to a ChildRelativeLocation.
	 *
	 * eg:
	 ...
	 * This converts a ChildRelativeTransform to a ChildWorldLocation.
	 *
	 * eg:
	 * FTransform ChildRelativeTransform = FTransform::GetRelative(ParentWorldTransform, ChildWorldTransform);
	 * FTransform ChildWorldTransform = FTransform::ApplyRelative(ParentWorldTransform, ChildRelativeTransform);
