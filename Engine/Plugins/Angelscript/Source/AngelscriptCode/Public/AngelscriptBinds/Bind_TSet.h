...
struct ANGELSCRIPTCODE_API FSetOperations
{
	...
	bool AnyEqualForHotReload(FScriptSet& Set, int32 Index, int32 Num, void* ElementToCompare)
	{
		for (; Num; --Num)
		{
			while (!Set.IsValidIndex(Index))
			...
			if (Type.IsValueEqualForHotReload(GetElement(Set, Index), ElementToCompare))
				return true;
	...
	bool RangesContainSameAmountsOfValForHotReload(FScriptSet& SetA, int32 IndexA, FScriptSet& SetB, int32 IndexB, int32 Num, void* ElementToCompare)
	{
		int32 CountA = 0;
		int32 CountB = 0;
		for (;;)
		{
			...
			void* ElementA = GetElement(SetA, IndexA);
			void* ElementB = GetElement(SetB, IndexB);
			if (Type.IsValueEqualForHotReload(ElementA, ElementToCompare))
				++CountA;

			if (Type.IsValueEqualForHotReload(ElementB, ElementToCompare))
				++CountB;
	...
	bool IsPermutationForHotReload(FScriptSet& SetA, FScriptSet& SetB)
	{
		int32 Num = SetA.Num();
		if (Num != SetB.Num())
			return false;
		...
		for (;;)
		{
			...
			void* ElementA = GetElement(SetA, IndexA);
			void* ElementB = GetElement(SetB, IndexB);
			if (!Type.IsValueEqualForHotReload(ElementA, ElementB))
				break;
		...
		{
			void* ElementA = GetElement(SetA, IndexA);
			if (!AnyEqualForHotReload(SetA, FirstIndexA, FirstNum - Num, ElementA) && !RangesContainSameAmountsOfValForHotReload(SetA, IndexA, SetB, IndexB, Num, ElementA))
				return false;
