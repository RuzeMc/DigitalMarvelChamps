...
struct ANGELSCRIPTCODE_API FMapOperations
{
	...
	bool AnyEqualForHotReload(FScriptMap& Map, int32 Index, int32 Num, void* PairToCompare)
	{
		for (; Num; --Num)
		{
			while (!Map.IsValidIndex(Index))
			...
			if (KeyType.IsValueEqualForHotReload(GetPair(Map, Index), PairToCompare)
				&& ValueType.IsValueEqualForHotReload(GetValue(Map, Index), GetValueFromPair(PairToCompare)))
				return true;
	...
	bool RangesContainSameAmountsOfValForHotReload(FScriptMap& MapA, int32 IndexA, FScriptMap& MapB, int32 IndexB, int32 Num, void* PairToCompare)
	{
		int32 CountA = 0;
		int32 CountB = 0;
		for (;;)
		{
			...
			if (PairA == PairToCompare || (KeyType.IsValueEqualForHotReload(PairA, PairToCompare)
					&& ValueType.IsValueEqualForHotReload(GetValueFromPair(PairA), GetValueFromPair(PairToCompare))))
				++CountA;

			if (PairB == PairToCompare || (KeyType.IsValueEqualForHotReload(PairB, PairToCompare)
					&& ValueType.IsValueEqualForHotReload(GetValueFromPair(PairB), GetValueFromPair(PairToCompare))))
				++CountB;
	...
	bool IsPermutationForHotReload(FScriptMap& MapA, FScriptMap& MapB)
	{
		int32 Num = MapA.Num();
		if (Num != MapB.Num())
			return false;
		...
		for (;;)
		{
			...
			if (!KeyType.IsValueEqualForHotReload(PairA, PairB))
				break;

			if (!ValueType.IsValueEqualForHotReload(GetValueFromPair(PairA), GetValueFromPair(PairB)))
				break;
		...
		{
			void* PairA = GetPair(MapA, IndexA);
			if (!AnyEqualForHotReload(MapA, FirstIndexA, FirstNum - Num, PairA) && !RangesContainSameAmountsOfValForHotReload(MapA, IndexA, MapB, IndexB, Num, PairA))
				return false;
