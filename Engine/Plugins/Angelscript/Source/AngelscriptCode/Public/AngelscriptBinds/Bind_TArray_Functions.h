...
#if DO_CHECK
	FORCEINLINE static void CheckAddress(FScriptArray& Arr, asCObjectType* Meta, void* Addr)
	{
		auto* Ops = GetArrayOperations(Meta);
		if ((SIZE_T)Addr >= (SIZE_T)Arr.GetData() && (SIZE_T)Addr < ((SIZE_T)Arr.GetData() + (SIZE_T)Arr.ArrayMax * Ops->NumBytesPerElement))
		{
			FAngelscriptManager::Throw("Attempting to use a container element which already comes from the container being modified.");
		}
	}
#endif
