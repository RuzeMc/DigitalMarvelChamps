...
void FAngelscriptType::Register(TSharedRef<FAngelscriptType> Type)
{
	...
	if (Class != nullptr)
	{
		if (!Database.TypesByClass.Contains(Class))
		{
	...
	if (Data != nullptr)
	{
		if (!Database.TypesByData.Contains(Data))
		{
...
{
	int32 TypeId = ((asCScriptEngine*)FAngelscriptManager::Get().Engine)->GetTypeIdFromDataType(DataType);
...
FAngelscriptTypeUsage FAngelscriptTypeUsage::FromTypeId(int32 TypeId)
{
	...
	if(!Usage.Type.IsValid())
	{
		...
		if (ScriptType != nullptr)
		{
			...
			if (ScriptType->flags & asOBJ_SCRIPT_OBJECT)
			{
				if (ScriptType->flags & asOBJ_VALUE)
				{
					void* UserData = (void*)ScriptType->plainUserData;
			...
			if (ScriptType->flags & asOBJ_ENUM)
			{
				if (ScriptType->module != nullptr)
				{
			...
			if (ScriptType->flags & asOBJ_VALUE)
			{
				...
				for (int32 i = 0, SubTypeCount = ObjectType->templateSubTypes.GetLength(); i < SubTypeCount; ++i)
				{
					FAngelscriptTypeUsage SubTypeUsage = FAngelscriptTypeUsage::FromDataType(ObjectType->templateSubTypes[i]);
					if (!SubTypeUsage.IsValid())
					{
						Usage.Type = nullptr;
					...
					Usage.SubTypes.Add(MoveTemp(SubTypeUsage));
				}
			}
		}
	}
...
bool FAngelscriptType::GetDebuggerValueFromFunction(asIScriptFunction* InScriptFunction, void* Object, FDebuggerValue& OutValue, class asITypeInfo* ContainerScriptType, UStruct* ContainerClass, const FString& PropertyAddrToSearchFor)
{
	...
	UAngelscriptSettings* Config = FAngelscriptManager::Get().ConfigSettings;
	...
	if (ReturnValue.IsValid() && ReturnValue.CanCopy() && ReturnValue.CanBeReturned())
	{
...

		{
			...
			if (bHasWorldContext)
				Context->SetArgObject(0, FAngelscriptManager::CurrentWorldContext);
			Context->Execute();
