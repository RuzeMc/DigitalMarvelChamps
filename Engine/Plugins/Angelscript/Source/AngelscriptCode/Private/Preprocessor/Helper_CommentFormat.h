...
inline FString FormatCommentForToolTip(const FString& Input)
{
	...
	// Sweep out comments marked to be ignored.
	{
		...
		for (CommentStart = Result.Find(TEXT("/*~")); CommentStart != INDEX_NONE; CommentStart = Result.Find(TEXT("/*~")))
		{
			...
			{
				Result.RemoveAt(CommentStart, (CommentEnd + 2) - CommentStart, EAllowShrinking::No);
			}
			else
			{
		...
		for (CommentStart = Result.Find(TEXT("//~")); CommentStart != INDEX_NONE; CommentStart = Result.Find(TEXT("//~")))
		{
			...
			if (CommentEnd != INDEX_NONE)
			{
				Result.RemoveAt(CommentStart, (CommentEnd + 1) - CommentStart, EAllowShrinking::No);
			}
			else
			{
				Result.RemoveAt(CommentStart, Result.Len() - CommentStart, EAllowShrinking::No);
				break;
			}
		}
