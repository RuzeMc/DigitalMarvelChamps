...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_FVector2D(FAngelscriptBinds::EOrder::Early, []
{
	...
	FVector2D_.SetTypeUserData(TBaseStructure<FVector2D>::Get());
	...
	FVector2D_.Method("void ToDirectionAndLength(FVector2D& OutDir, float64& OutLength) const", METHODPR_TRIVIAL(void, FVector2D, ToDirectionAndLength, (FVector2D&, double&) const));
	FVector2D_.Method("void ToDirectionAndLength(FVector2D& OutDir, float32& OutLength) const", METHODPR_TRIVIAL(void, FVector2D, ToDirectionAndLength, (FVector2D&, float&) const));
