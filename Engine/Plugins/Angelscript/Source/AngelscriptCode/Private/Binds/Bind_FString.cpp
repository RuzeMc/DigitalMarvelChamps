...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_FString(FAngelscriptBinds::EOrder::Early, []
{
	...
	FString_.Method("void RemoveAt(int Index, int Count)", [](FString& String, int32 Index, int32 Count) -> void
	{
		if (!String.IsValidIndex(Index))
		{
			FAngelscriptManager::Throw("String index out of bounds.");
...
void FToStringHelper::Generic_AppendToString(FString& AppendTo, void* ValuePtr, int TypeId)
{
	...
	if (TypeInfo != nullptr && (TypeInfo->GetFlags() & asOBJ_VALUE))
	{
		...
		UStruct* TypeStruct = FAngelscriptManager::Get().GetUnrealStructFromAngelscriptTypeId(TypeId);
		if (UASStruct* CastStruct = Cast<UASStruct>(TypeStruct))
		{
			if (asIScriptFunction* ToStrFn = CastStruct->GetToStringFunction())
			{
				FAngelscriptContext Context;
				Context->Prepare(ToStrFn);
				Context->SetObject(ValuePtr);
				Context->Execute();
				FString* ReturnString = (FString*)Context->GetReturnObject();
				if (ReturnString != nullptr)
				{
					AppendTo += *ReturnString;
					return;
				}
			}
		}
