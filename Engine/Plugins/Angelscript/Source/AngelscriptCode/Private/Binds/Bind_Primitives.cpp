...
struct FBoolType : TPrimitiveAngelscriptType<bool, FBoolProperty>
{
	...
	bool BindProperty(const FAngelscriptTypeUsage& Usage, const FBindParams& Params, FProperty* NativeProperty) const override
	{
		...
		if (Params.bCanRead && !BindClass.HasGetter(NativeProperty->GetName(), true))
		{
			...
			// Only if bAllowImplicitPropertyAccessors is false since it ends up being set as such anyways in that case and
			// can cause issues with the function.traits if called twice unnecessarily
			if (!FAngelscriptManager::Get().ConfigSettings->bAllowImplicitPropertyAccessors)
				BindClass.SetPreviousBindIsPropertyAccessor(true);
		...
		if ((Params.bCanWrite || Params.bCanEdit) && !BindClass.HasSetter(NativeProperty->GetName(), true))
		{
