	...
	bool GetCppForm(const FAngelscriptTypeUsage& Usage, FCppForm& OutCppForm) const override
	{
		if (Struct == nullptr)
		{
			...
			if (ObjectType != nullptr && (ObjectType->flags & asOBJ_POD) != 0
				&& FAngelscriptManager::Get().StaticJIT != nullptr
				&& !FAngelscriptManager::Get().StaticJIT->IsTypePotentiallyDifferent(ObjectType)
...
static void BindStructBehaviors(FAngelscriptBinds& Binds, const FString& TypeName, UScriptStruct* Struct)
{
	...
	if (Ops != nullptr)
	{
		...
		else if (Ops->HasZeroConstructor())
		{
			Binds.Constructor("void f()", [](void* Ptr)
			{
				UScriptStruct::ICppStructOps* Ops = FAngelscriptManager::GetCurrentFunctionUserData<UScriptStruct::ICppStructOps>();
	...
	else
	{
		...
		Binds.Constructor("void f()", [](void* Ptr)
		{
			UScriptStruct* Struct = FAngelscriptManager::GetCurrentFunctionUserData<UScriptStruct>();
...
bool ShouldBindEngineType(UScriptStruct* Struct)
{
	...
	// Don't bind any base types we've already bound manually
