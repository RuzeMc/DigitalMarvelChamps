...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_FComponentQueryParams_Late(FAngelscriptBinds::EOrder::Late, []
{
	...
	FComponentQueryParams_.Method("TArray<uint32> GetIgnoredActors() const",
		[](FComponentQueryParams* Address) -> TArray<uint32>
		{
			const FComponentQueryParams::IgnoreActorsArrayType& IgnoredActors = Address->GetIgnoredActors();
			TArray<uint32> OutArray;
			OutArray.SetNumUninitialized(IgnoredActors.Num());
			FMemory::Memcpy(OutArray.GetData(), IgnoredActors.GetData(), IgnoredActors.Num() * sizeof(IgnoredActors[0]));
			return OutArray;
	...
	FComponentQueryParams_.Method("void ClearIgnoredActors()", METHOD_TRIVIAL(FComponentQueryParams, ClearIgnoredActors));
	...
	FComponentQueryParams_.Method("void AddIgnoredActor(const AActor InIgnoreActor)", METHODPR_TRIVIAL(void, FComponentQueryParams, AddIgnoredActor, (const AActor*)));
	FComponentQueryParams_.Method("void AddIgnoredActor(const uint32 InIgnoreActorID)", METHODPR_TRIVIAL(void, FComponentQueryParams, AddIgnoredActor, (const uint32)));
	FComponentQueryParams_.Method("void AddIgnoredActors(const TArray<AActor>& InIgnoreActors)", METHODPR_TRIVIAL(void, FComponentQueryParams, AddIgnoredActors, (const TArray<AActor*>&)));
	FComponentQueryParams_.Method("void AddIgnoredActors(const TArray<const AActor>& InIgnoreActors)", METHODPR_TRIVIAL(void, FComponentQueryParams, AddIgnoredActors, (const TArray<const AActor*>&)));
	FComponentQueryParams_.Method("void AddIgnoredComponent(const UPrimitiveComponent InIgnoreComponent)", METHOD_TRIVIAL(FComponentQueryParams, AddIgnoredComponent));
	FComponentQueryParams_.Method("void AddIgnoredComponents(const TArray<UPrimitiveComponent>& InIgnoreComponents)", METHODPR_TRIVIAL(void, FComponentQueryParams, AddIgnoredComponents, (const TArray<UPrimitiveComponent*>&)));
	FComponentQueryParams_.Method("void AddIgnoredComponent_LikelyDuplicatedRoot(const UPrimitiveComponent InIgnoreComponent)", METHOD_TRIVIAL(FComponentQueryParams, AddIgnoredComponent_LikelyDuplicatedRoot));
	FComponentQueryParams_.Method("FString ToString() const", METHOD_TRIVIAL(FComponentQueryParams, ToString));
});
