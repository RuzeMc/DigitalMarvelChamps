...
UActorComponent* FAngelscriptActorBinds::GetComponent(const AActor* OnActor, const TSubclassOf<UActorComponent>& ComponentClass, const FName& WithName)
{
	if (OnActor == nullptr)
	{
		FAngelscriptManager::Throw("Actor was null.");
...
void FAngelscriptActorBinds::GetAllComponents(const AActor* OnActor, UClass* ComponentClass, TArray<UActorComponent*>& OutComponents)
{
	if (OnActor == nullptr)
	{
		FAngelscriptManager::Throw("Actor was null.");
...
UActorComponent* FAngelscriptActorBinds::GetComponentFromMeta(class asCScriptFunction* Meta, const AActor* OnActor, const FName& WithName)
{
	if (OnActor == nullptr)
	{
		FAngelscriptManager::Throw("Actor was null.");
...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_Components((int32)FAngelscriptBinds::EOrder::Late + 150, []
{
	...
	AActor_.Method("UActorComponent GetComponent(const TSubclassOf<UActorComponent>& ComponentClass, const FName& WithName = NAME_None) const", FUNC(FAngelscriptActorBinds::GetComponent));
	FAngelscriptBinds::SetPreviousBindArgumentDeterminesOutputType(0);
	...
	AActor_.Method("void GetAllComponents(UClass ComponentClass, TArray<UActorComponent>& OutComponents) const", FUNC(FAngelscriptActorBinds::GetAllComponents));
