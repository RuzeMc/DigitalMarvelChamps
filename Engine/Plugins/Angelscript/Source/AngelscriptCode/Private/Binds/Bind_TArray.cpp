...
bool FAngelscriptArrayType::CanCompareForHotReload(const FAngelscriptTypeUsage& Usage) const 
{
	return Usage.SubTypes.Num() == 1 && Usage.SubTypes[0].CanCompareForHotReload();
}

bool FAngelscriptArrayType::IsValueEqualForHotReload(const FAngelscriptTypeUsage& Usage, void* SourcePtr, void* DestinationPtr) const 
{
	const FAngelscriptTypeUsage& SubType = Usage.SubTypes[0];
	FScriptArray& SourceArray = *(FScriptArray*)SourcePtr;
	FScriptArray& DestArray = *(FScriptArray*)DestinationPtr;
	...
	for (int32 i = 0; i < SourceNum; ++i)
	{
		...
		if (!SubType.IsValueEqualForHotReload(SourceValue, DestValue))
			return false;
	}
...
bool FAngelscriptArrayType::GetDebuggerScope(const FAngelscriptTypeUsage& Usage, void* Address, struct FDebuggerScope& Scope) const
{
...

	{
		...
		NumValue.SetAddressToMonitor(&Array.ArrayNum, sizeof(int32));
