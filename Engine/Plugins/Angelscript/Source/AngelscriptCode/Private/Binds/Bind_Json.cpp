...
struct FJsonObjectContainer
{
	...
	bool TryGetBoolField(const FString& FieldName, bool& bOutBool) const
	{
		if (CheckValidObject())
		{
			auto Value = JsonObject->GetField<EJson::None>(FieldName);
			return Value->TryGetBool(bOutBool);
		}
		else
		{
			return false;
...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_JsonClasses(FAngelscriptBinds::EOrder::Late, []
	{
		...
		JsonValueArray.Method("void AddBoolean(bool B)", &FJsonValueArrayContainer::AddBoolean);
		JsonValueArray.Method("void AddArray(FJsonArray Array)", &FJsonValueArrayContainer::AddArray);
		...
		JsonObject.Method("bool TryGetStringField(const FString& FieldName, FString& OutString) const", &FJsonObjectContainer::TryGetStringField);
		JsonObject.Method("bool TryGetNumberField(const FString& FieldName, float64& OutNumber) const", &FJsonObjectContainer::TryGetNumberField);
		JsonObject.Method("bool TryGetBoolField(const FString& FieldName, bool& bOutBool) const", &FJsonObjectContainer::TryGetBoolField);
		...
		JsonValueArray.Method("void AddObject(FJsonObject Object)", &FJsonValueArrayContainer::AddObject);
