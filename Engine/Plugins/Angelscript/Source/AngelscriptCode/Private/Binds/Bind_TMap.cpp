...
struct FAngelscriptMapType : public FAngelscriptType
{
	...
	bool GetCppForm(const FAngelscriptTypeUsage& Usage, FCppForm& OutCppForm) const override
	{
		...
		bool bNativizeAllowed = true;
		if (const UScriptStruct* KeyStruct = Cast<UScriptStruct>(Usage.SubTypes[0].GetUnrealStruct()))
		{
			if (KeyStruct->GetCppStructOps() == nullptr || !KeyStruct->GetCppStructOps()->HasGetTypeHash() || KeyStruct->IsA<UASStruct>() || !Usage.SubTypes[0].CanCompare() || (KeyStruct->StructFlags & STRUCT_IdenticalNative) == 0)
			{
				bNativizeAllowed = false;
			}
		}

		FCppForm CppInnerKey;
		FCppForm CppInnerValue;
		if (bNativizeAllowed && Usage.SubTypes[0].GetCppForm(CppInnerKey) && Usage.SubTypes[1].GetCppForm(CppInnerValue))
		{
	...
	{
		return Usage.SubTypes.Num() == 2 && Usage.SubTypes[0].CanCompareForHotReload() && Usage.SubTypes[1].CanCompareForHotReload();
	}
	...
	bool IsValueEqualForHotReload(const FAngelscriptTypeUsage& Usage, void* SourcePtr, void* DestinationPtr) const override
	{
		...
		FMapOperations Ops(KeyType, ValueType);
		return Ops.IsPermutationForHotReload(SourceMap, DestMap);
	}
};
...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_TMap((int)FAngelscriptBinds::EOrder::Early+1, []
{
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Destruct", false);
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::OpIndex", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Add", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TMap_.Method("bool Contains(const K&in if_handle_then_const Key) const", &FAngelscriptMapBinds::Contains);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Contains", true);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::RemoveAndCopyValue", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TMap_.Method("bool Remove(const K&in if_handle_then_const Key)", &FAngelscriptMapBinds::Remove);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Remove", true);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::FindOrAdd", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Find", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::OpAssign", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TMap_.Method("void Empty(int32 Slack = 0)", &FAngelscriptMapBinds::Empty);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Empty", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TMap_.Method("void Reset()", &FAngelscriptMapBinds::Reset);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TMap_, "FAngelscriptMapBinds::Reset", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
