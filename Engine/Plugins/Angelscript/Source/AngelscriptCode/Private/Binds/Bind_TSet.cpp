...
struct FAngelscriptSetType : public FAngelscriptType
{
	...
	bool GetCppForm(const FAngelscriptTypeUsage& Usage, FCppForm& OutCppForm) const override
	{
		...
		if (const UScriptStruct* SubStruct = Cast<UScriptStruct>(Usage.SubTypes[0].GetUnrealStruct()))
		{
			if (SubStruct->GetCppStructOps() == nullptr || !SubStruct->GetCppStructOps()->HasGetTypeHash() || SubStruct->IsA<UASStruct>() || !Usage.SubTypes[0].CanCompare() || (SubStruct->StructFlags & STRUCT_IdenticalNative) == 0)
			{
		...
		FCppForm CppInner;
		if (bNativizeAllowed && Usage.SubTypes[0].GetCppForm(CppInner))
		{
	...
	{
		return Usage.SubTypes.Num() == 1 && Usage.SubTypes[0].CanCompareForHotReload();
	}
	...
	bool IsValueEqualForHotReload(const FAngelscriptTypeUsage& Usage, void* SourcePtr, void* DestinationPtr) const override
	{
		...
		check(SubType.CanCompareForHotReload());

		FSetOperations Ops(SubType);
		return Ops.IsPermutationForHotReload(SourceSet, DestSet);
	}
};
...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_TSet((int32)FAngelscriptBinds::EOrder::Early+1, []
{
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Destruct", false);
	...
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Add", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("void Append(const TArray<T>& Array)", &FAngelscriptSetBinds::AppendArray);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::AppendArray", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("void Append(const TSet<T>& Set)", &FAngelscriptSetBinds::AppendSet);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::AppendSet", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("bool Contains(const T&in if_handle_then_const Value) const", &FAngelscriptSetBinds::Contains);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Contains", true);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("bool Remove(const T&in if_handle_then_const Value)", &FAngelscriptSetBinds::Remove);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Remove", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("TSet<T>& opAssign(const TSet<T>& Other)", &FAngelscriptSetBinds::Assign);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Assign", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("void Empty(int32 Slack = 0)", &FAngelscriptSetBinds::Empty);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Empty", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();

	TSet_.Method("void Reset()", &FAngelscriptSetBinds::Reset);
	SCRIPT_NATIVE_TEMPLATED_CALL_NEEDS_HASHKEY(TSet_, "FAngelscriptSetBinds::Reset", false);
	FAngelscriptBinds::PreviousBindPassScriptObjectTypeAsFirstParam();
