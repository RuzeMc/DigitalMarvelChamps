...
void AssetManager_LoadPrimaryAssetWithType(UAssetManager* AssetManager, FPrimaryAssetType PrimaryAssetType, const TArray<FName>& LoadBundles, int32 Priority, UObject* OptionalCallbackObject, FName OptionalFinishedCallbackFunctionName, FName OptionalCanceledCallbackName)
{
	FStreamableDelegate CompleteDelegate{};
	if (OptionalCallbackObject != nullptr && !OptionalFinishedCallbackFunctionName.IsNone())
	{
		// We bind the complete delegate here since it can be called by LoadPrimaryAssetsWithType if there's no work to be done.
		CompleteDelegate.BindUFunction(OptionalCallbackObject, OptionalFinishedCallbackFunctionName);
	}

	TSharedPtr<FStreamableHandle> Handle = AssetManager->LoadPrimaryAssetsWithType(PrimaryAssetType, LoadBundles, CompleteDelegate, Priority);
...
AS_FORCE_LINK const FAngelscriptBinds::FBind Bind_AssetManager(FAngelscriptBinds::EOrder::Late, []
{
	...
	UAssetManager_.Method("void LoadPrimaryAssetsWithType(FPrimaryAssetType PrimaryAssetType, const TArray<FName>& LoadBundles = TArray<FName>(), int32 Priority = 0, UObject OptionalCallbackObject = nullptr, FName OptionalFinishedCallbackFunctionName = NAME_None, FName OptionalCanceledCallbackName = NAME_None)",
	[](UAssetManager* AssetManager, FPrimaryAssetType PrimaryAssetType, const TArray<FName>& LoadBundles, int32 Priority, UObject* OptionalCallbackObject, FName OptionalFinishedCallbackFunctionName, FName OptionalCanceledCallbackName)
	{
		AssetManager_LoadPrimaryAssetWithType(AssetManager, PrimaryAssetType, LoadBundles, Priority, OptionalCallbackObject, OptionalFinishedCallbackFunctionName, OptionalCanceledCallbackName);
	});
	{
