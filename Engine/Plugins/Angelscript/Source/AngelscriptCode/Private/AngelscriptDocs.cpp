...
void FAngelscriptDocs::AddScriptFunctionMeta(int FunctionId, FStringView MetaName, FStringView MetaValue)
{
	FPassedDoc& Doc = UnrealDocumentation.FindOrAdd(FunctionId);
	Doc.Meta.Add(FString(MetaName), FString(MetaValue));
}

const TMap<FString, FString>* FAngelscriptDocs::GetScriptFunctionMeta(int FunctionId)
{
	FPassedDoc* Doc = UnrealDocumentation.Find(FunctionId);
	if (Doc != nullptr)
		return &Doc->Meta;
	return nullptr;
}
