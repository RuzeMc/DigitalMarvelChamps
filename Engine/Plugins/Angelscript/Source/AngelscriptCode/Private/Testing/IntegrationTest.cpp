...
void ConfigureEditorForTest(FEditorState& OutOriginalState)
{
	...
	// The original implementer was using a client-server model
	// (EPlayNetMode::PIE_Client) for all tests, which breaks a lot of singleplayer game
	// things. Allow changing it to Standalone which is the expected model for singleplayer.
	if (TestSettings->bUseServerClientModel)
	{
	...
	FAngelscriptTestDesc* OutTestFuncDesc, FString* OutMapObjectPath)
{
	...
	*OutMapObjectPath = MapRoot + OutTestFuncDesc->Function->GetName();
	...
	// if a function with the name <IntegrationTest_FunctionName>_GetMapName that returns an FString
	// exists, use that as the map instead. The returned map path requires the format
	// "/Game/ExampleProject/Maps/ExampleMap".
	FString MapNameFunctionDecl = FString::Printf(TEXT("FString %s_GetMapName()"), *TestFunctionName);
	const auto* ScriptModule = (*OutModule)->ScriptModule;
	if (ScriptModule)
	{
		if (auto GetMapNameFunc = ScriptModule->GetFunctionByDecl(TCHAR_TO_ANSI(*MapNameFunctionDecl)))
		{
			FAngelscriptContext AngelscriptContext = FAngelscriptContext();
			AngelscriptContext->Prepare(GetMapNameFunc);
			AngelscriptContext->Execute();
			FString MapName = *(FString*)AngelscriptContext->GetReturnObject();
			*OutMapObjectPath = MapName;
		}
	}
