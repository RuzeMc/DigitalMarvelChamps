...
FNativeFunctionCall GenerateCustomCall_PushArg(FNativeFunctionContext& Context, FStaticJITContext& JITContext, bool bIsRef)
{
	...
	FStaticJITContext::FPropertyInParm& PropInParm = JITContext.PropertiesInParm.AddDefaulted_GetRef();
	...
	if (bIsRef)
	{
		// if the arg we're pushing is a reference, copy the original argument address into a local variable
		PropInParm.SourceExpr = FString::Printf(TEXT("l_ParmStructOutAddr_%d"), ParmOffset);

		JITContext.AddStateVar("void*", PropInParm.SourceExpr);
		JITContext.Line("{0} = (void*){1};", PropInParm.SourceExpr, Context.ArgumentValues[0]);
	}

	const FString Source = bIsRef ? PropInParm.SourceExpr : Context.ArgumentValues[0];
	...
	if (DataType.IsPrimitive() || DataType.IsEnumType())
	{
		...
		{
		case 1:
			JITContext.Line("(asBYTE&)l_ParmStruct[{0}] = *(asBYTE*){1};", ParmOffset, Source);
		break;
		case 2:
			JITContext.Line("(asWORD&)l_ParmStruct[{0}] = *(asWORD*){1};", ParmOffset, Source);
		break;
		case 4:
			JITContext.Line("(asDWORD&)l_ParmStruct[{0}] = *(asDWORD*){1};", ParmOffset, Source);
		break;
		case 8:
			JITContext.Line("(asQWORD&)l_ParmStruct[{0}] = *(asQWORD*){1};", ParmOffset, Source);
		break;
		}
	...
	{
		JITContext.Line("(UObject*&)l_ParmStruct[{0}] = *(UObject**){1};", ParmOffset, Source);
...
void CleanupPropertiesInParm(FNativeFunctionContext& Context, FStaticJITContext& JITContext)
{
	...
	for (auto& PropInParm : JITContext.PropertiesInParm)
	{
		...
		{
			check(!PropInParm.SourceExpr.IsEmpty());
			if (DataType.IsPrimitive() || DataType.IsEnumType())
			{
				switch (DataType.GetSizeInMemoryBytes())
				{
...
IMPL_BYTECODE_BEGIN(asBC_PshV1)
{
	bool Implement(FStaticJITContext& Context) const override
	{
...
IMPL_BYTECODE_BEGIN(asBC_BSRA)
{
	bool Implement(FStaticJITContext& Context) const override
	{
		...
		check(Context.VArg_IsNumeric(1));
		check(Context.VArg_IsNumeric(2));
		Context.Line("{0} = {1} >> {2};", Context.VArg_SignedVar(0), Context.VArg_SignedVar(1), Context.VArg_Var(2));
		return true;
	}
};
...
IMPL_BYTECODE_BEGIN(asBC_STOREOBJ)
{
	...
	{
		check(Context.VArg_Type(0) == FStaticJITContext::EVariableType::Pointer);
		Context.Line("memcpy({0}, &l_objectRegister, sizeof(void*));", Context.VArg_Address(0));
...
IMPL_BYTECODE_BEGIN(asBC_TrackRef)
{
	bool Implement(FStaticJITContext& Context) const override
	{
		return true;
...
IMPL_BYTECODE_END(asBC_TrackRef)

IMPL_BYTECODE_BEGIN(asBC_UntrackRef)
{
	bool Implement(FStaticJITContext& Context) const override
	{
		return true;
...
IMPL_BYTECODE_END(asBC_UntrackRef)

IMPL_BYTECODE_BEGIN(asBC_ValidateRef)
{
	bool Implement(FStaticJITContext& Context) const override
	{
		return true;
...
IMPL_BYTECODE_END(asBC_ValidateRef)
...
void ByteCode_CpyRToV(FStaticJITContext& Context)
{
	...
	case FStaticJITContext::EVariableType::Double:
	{
		...
		{
			Context.MaterializeValueRegister();
			Context.Line("memcpy({0}, &l_valueRegister, 8);", Context.VArg_Address(0));
		}
	}
	break;
	default:
...
IMPL_BYTECODE_BEGIN(asBC_POWf)
{
	bool Implement(FStaticJITContext& Context) const override
	{
		...
		Context.Line("{0} = FMath::Pow({1}, {2});", Context.VArg_Var(0), Context.VArg_Var(1), Context.VArg_Var(2));
		return true;
	}
};
...
IMPL_BYTECODE_BEGIN(asBC_POWdi)
{
	bool Implement(FStaticJITContext& Context) const override
	{
		...
		Context.Line("{0} = FMath::Pow({1}, (double){2});", Context.VArg_Var(0), Context.VArg_Var(1), Context.VArg_SignedVar(2));
		return true;
	}
};
