...
asCString GetScriptClassName(asITypeInfo* Type)
{
	...
	if (SubTypeCount != 0)
	{
		...
		for (int32 i = 0; i < SubTypeCount; ++i)
		{
			if (i != 0)
				Str += ",";
			Str += FAngelscriptManager::Get().Engine->GetTypeDeclaration(Type->GetSubTypeId(i));
		}
		Str += ">";
	}
	return Str;
...
FString FStaticJITContext::ReferenceTypeId(asDWORD TypeId)
{
	...
	asDWORD ExtraFlags = ((asDWORD)TypeId) & ~(asTYPEID_MASK_SEQNBR | asTYPEID_MASK_OBJECT);
	if (ExtraFlags != 0)
	{
...
bool FAngelscriptStaticJIT::IsTypePotentiallyDifferent(asITypeInfo* TypeInfo)
{
	...
	int32 Flags = ObjectType->GetFlags();
