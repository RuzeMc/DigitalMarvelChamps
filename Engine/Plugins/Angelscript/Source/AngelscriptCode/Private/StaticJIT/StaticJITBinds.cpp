...
struct FScriptNativeTemplateInstantiation : public FScriptFunctionNativeForm
{
	...
	FScriptNativeTemplateInstantiation(const ANSICHAR* InName, bool InTrivial, bool InNeedsCompare, bool InNeedsCopy, bool InNeedsHash)
		: Name(InName), bTrivial(InTrivial), bNeedsCompare(InNeedsCompare), bNeedsCopy(InNeedsCopy), bNeedsHash(InNeedsHash)
	{
	}
	...
	FString GetNativeObjectType(FNativeFunctionContext& Context) const
	{
		...
		for (int i = 0, Count = Context.ObjectType.Usage.SubTypes.Num(); i < Count; ++i)
		{
			const FAngelscriptTypeUsage& SubType = Context.ObjectType.Usage.SubTypes[i];
			...
			if (SubStruct != nullptr)
			{
				...
				if (bNeedsHash && (i == 0))
				{
					if (SubStruct->GetCppStructOps() == nullptr || !SubStruct->GetCppStructOps()->HasGetTypeHash() || SubStruct->IsA<UASStruct>() || !SubType.CanCompare() || (SubStruct->StructFlags & STRUCT_IdenticalNative) == 0)
					{
						bSubtypesAllowed = false;
					}
				}
...
void FScriptFunctionNativeForm::BindTemplateInstantiatedCall(FAngelscriptBinds& Binds, const ANSICHAR* Name, bool bTrivial, bool bNeedsCompare, bool bNeedsCopy, bool bNeedsHash)
{
	if (!FAngelscriptManager::bGeneratePrecompiledData)
		return;
	GScriptNativeForms.Add(FAngelscriptBinds::GetPreviousBind(), new FScriptNativeTemplateInstantiation(Name, bTrivial, bNeedsCompare, bNeedsCopy, bNeedsHash));
}
