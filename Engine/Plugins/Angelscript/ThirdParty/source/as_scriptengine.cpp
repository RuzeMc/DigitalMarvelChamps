...
struct InScriptFunction
{
	asIScriptFunction* PrevFun;
	asCThreadLocalData* tld;

	inline InScriptFunction(const asCScriptEngine* engine, asCScriptFunction* Fun)
	{
		tld = asCThreadManager::GetLocalData();
		PrevFun = tld->activeFunction;
		tld->activeFunction = Fun;
	}

	inline ~InScriptFunction()
	{
		tld->activeFunction = PrevFun;
	}
};


inline asQWORD GetTemplateInstanceBucket(asCObjectType* templateType, const asCArray<asCDataType>& subTypes)
{
	asQWORD Bucket = (asQWORD)templateType;
	for (asUINT n = 0; n < subTypes.GetLength(); n++)
	{
		Bucket ^= (asQWORD)subTypes[n].GetTokenType();
		Bucket ^= (asQWORD)subTypes[n].GetTypeInfo();
	}
	return Bucket;
}
...
int asCScriptEngine::SetEngineProperty(asEEngineProp property, asPWORD value)
{
	...
	case asEP_AUTOMATIC_IMPORTS:
		ep.automaticImports = value ? true : false;
		break;

	case asEP_TYPECHECK_SWITCH_ENUMS:
		ep.typeCheckSwitchEnums = value ? true : false;
		break;

	case asEP_ALLOW_DOUBLE_TYPE:
		ep.allowDoubleType = value ? true : false;
		break;

	case asEP_FLOAT_IS_FLOAT64:
		ep.floatIsFloat64 = value ? true : false;
		asCDataType::floatIsFloat64 = value ? true : false;
		break;

	case asEP_WARN_ON_FLOAT_CONSTANTS_FOR_DOUBLES:
		ep.warnOnFloatConstantsForDoubles = value ? true : false;
		break;

	case asEP_WARN_INTEGER_DIVISION:
		ep.warnIntegerDivision = value ? true : false;
		break;
...
asPWORD asCScriptEngine::GetEngineProperty(asEEngineProp property) const
{
	...
	case asEP_ALLOW_DOUBLE_TYPE:
		return ep.allowDoubleType;

	case asEP_FLOAT_IS_FLOAT64:
		return ep.floatIsFloat64;

	case asEP_WARN_ON_FLOAT_CONSTANTS_FOR_DOUBLES:
		return ep.warnOnFloatConstantsForDoubles;

	case asEP_WARN_INTEGER_DIVISION:
		return ep.warnIntegerDivision;
...
asIScriptFunction *asCScriptEngine::CreateDelegate(asIScriptFunction *func, void *obj)
{
	...
	return AS_NAMESPACE_QUALIFIER CreateDelegate(static_cast<asCScriptFunction*>(func), obj);
}
...
asCScriptEngine::asCScriptEngine()
{
	...
	// Engine properties
	{
		...
		ep.propertyAccessorMode          = 3;         // 0 = disable, 1 = app registered only, 2 = app and script created, 3 = flag with 'property' attribute
	...
	asASSERT( GetTypeIdFromDataType(asCDataType::CreatePrimitive(ttFloat32,  false)) == asTYPEID_FLOAT32  );
	asASSERT( GetTypeIdFromDataType(asCDataType::CreatePrimitive(ttFloat64, false)) == asTYPEID_FLOAT64 );
...
{
	for( int n = scriptModules.GetLength() - 1; n >= 0; n-- )
	{
	...
	for( int n = globalProperties.GetLength() - 1; n >= 0; n-- )
	{
...
asCScriptEngine::~asCScriptEngine()
{
	...
	mapTypeIdToTypeInfo.Reset();

	for( asUINT n = 0; n < registeredGlobalProps.GetLength(); n++ )
	{
		registeredGlobalProps[n]->Release();
	}
	registeredGlobalProps.SetLength(0);
	registeredGlobalPropTable.EraseAll();
	...
	for( asUINT n = 0; n < registeredGlobalFuncs.GetLength(); n++ )
	{
		registeredGlobalFuncs[n]->ReleaseInternal();
	}
	...
	scriptSectionIndexMap.IterateAll([](ScriptSectionName* SectionName)
	{
		asDELETE(SectionName, ScriptSectionName);
	});
...
{
	return nameSpacesByName.FindFirst(name);
}
...
int asCScriptEngine::SetMessageCallback(const asSFuncPtr &callback, void *obj, asDWORD callConv)
{
	...
	int r = DetectCallingConvention(isObj, callback, callConv, 0, nullptr, &msgCallbackFunc);
...
int asCScriptEngine::GetMethodIdByDecl(const asCObjectType *ot, const char *decl, asCModule *mod)
{
	...
	bool multiple = false;
	ot->FindMethod(func.name.AddressOf(), [&](asCScriptFunction* otherFunc)
	{
		if( func.IsSignatureExceptNameEqual(otherFunc) )
		{
			if (id == -1)
				id = otherFunc->id;
			else
	...
	if (multiple) return asMULTIPLE_FUNCTIONS;
...
// interface
int asCScriptEngine::RegisterObjectProperty(const char *obj, const char *declaration, int byteOffset, int compositeOffset, bool isCompositeIndirect, const asUINT accessorType, bool isProtected)
{
	int r;
	asCDataType dt;
	asCBuilder bld(this, 0);
	r = bld.ParseDataType(obj, &dt, defaultNamespace, false, true);
	if( r < 0 )
	...
	prop->exposedType         = accessorType;
	prop->isAppBindProperty = true;
		
	auto* objectType = CastToObjectType(dt.GetTypeInfo());
	objectType->properties.PushLast(prop);
	objectType->propertyTable.Add(prop);
...
int asCScriptEngine::RegisterInterface(const char *name)
{
	...
	int r = bld.ParseDataType(name, &dt, defaultNamespace, false, true);
	...
	allRegisteredTypes.Add(st);
	allRegisteredTypesByName.Add(st);
...
int asCScriptEngine::RegisterInterfaceMethod(const char *intf, const char *declaration)
{
	asCDataType dt;
	asCBuilder bld(this, 0);
	int r = bld.ParseDataType(intf, &dt, defaultNamespace, false, true);
	if( r < 0 )
	...
	func->objectType->methodTable.Add(func);
...
int asCScriptEngine::RegisterObjectType(const char *name, int byteSize, asDWORD flags)
{
	...
	else
	{
		...
		asCDataType dt;
		bool oldMsgCallback = msgCallback; msgCallback = false;
		r = bld.ParseDataType(name, &dt, defaultNamespace, false, true);
		msgCallback = oldMsgCallback;
...
// interface
int asCScriptEngine::RegisterObjectBehaviour(const char *datatype, asEBehaviours behaviour, const char *decl, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller, void *auxiliary, int compositeOffset, bool isCompositeIndirect)
{
	...
	asCBuilder bld(this, 0);
	asCDataType type;
	int r = bld.ParseDataType(datatype, &type, defaultNamespace, false, true);
	if( r < 0 )
	...
	return RegisterBehaviourToObjectType(CastToObjectType(type.GetTypeInfo()), behaviour, decl, funcPointer, callConv, caller, auxiliary, compositeOffset, isCompositeIndirect);
}

// internal
int asCScriptEngine::RegisterBehaviourToObjectType(asCObjectType *objectType, asEBehaviours behaviour, const char *decl, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller, void *auxiliary, int compositeOffset, bool isCompositeIndirect)
{
#ifdef AS_MAX_PORTABILITY
	...
	int r = DetectCallingConvention(isMethod, funcPointer, callConv, auxiliary, caller, &internal);
	if( r < 0 )
		return ConfigError(r, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl);

	if( (compositeOffset || isCompositeIndirect) )
	...
	asCBuilder bld(this, 0);
	r = bld.ParseFunctionDeclaration(objectType, decl, &func, true, nullptr, nullptr, 0, expectListPattern ? &listPattern : 0);
	if( r < 0 )
	{
		return ConfigError(asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl);
	}
	asCString name;
	name.Format("$beh%d", behaviour);
	func.name = name;
	...
	if( objectType->flags & asOBJ_TEMPLATE )
	{
		...
		// Mark the function as generic template function
		func.traits.SetTrait(asTRAIT_GENERIC_TEMPLATE_FUNCTION, true);
	}
	...
	if( behaviour == asBEHAVE_CONSTRUCT )
	{
		...
		else
		{
			...
			// Verify that the same constructor hasn't been registered already
			for (int i = beh->constructors.GetLength() - 1; i >= 0; --i)
			{
				auto* ExistingConstructor = GetScriptFunction(beh->constructors[i]);
				if (ExistingConstructor != nullptr)
				{
					if( func.IsSignatureExceptNameEqual(ExistingConstructor) )
					{
			...
			func.traits.SetTrait(asTRAIT_CONSTRUCTOR, true);
	...
	else if( behaviour == asBEHAVE_TEMPLATE_CALLBACK )
	{
		...
		if( func.inOutFlags[0] != asTM_INREF || func.inOutFlags[1] != asTM_OUTREF || !func.parameterTypes[1].IsEqualExceptRef(asCDataType::CreatePrimitive(ttInt, false)) )
			return ConfigError(asINVALID_DECLARATION, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl);
...
int asCScriptEngine::RegisterGlobalProperty(const char *declaration, void *pointer)
{
	...
	varAddressMap.Add(prop->GetAddressOfValue(), prop);

	registeredGlobalProps.PushLast(prop);
	registeredGlobalPropTable.Add(prop);
	prop->AddRef();
...
void asCScriptEngine::RemoveGlobalProperty(asCGlobalProperty *prop)
{
	...
	if( index >= 0 )
	{
		RemoveGlobalProperty(index);
	}
}

void asCScriptEngine::RemoveGlobalProperty(int index)
{
	auto* prop = globalProperties[index];
	freeGlobalPropertyIds.PushLast(index);
	globalProperties[index] = 0;

	varAddressMap.Remove(prop->GetAddressOfValue());
	prop->Release();
}
...
int asCScriptEngine::GetGlobalPropertyByIndex(asUINT index, const char **name, const char **nameSpace, int *typeId, bool *isConst, const char **configGroup, void **pointer, asDWORD *accessMask) const
{
	if (index >= registeredGlobalProps.GetLength())
		return asINVALID_ARG;
	const asCGlobalProperty *prop = registeredGlobalProps[index];
	if( !prop )
		return asINVALID_ARG;
...
// interface
int asCScriptEngine::RegisterObjectMethod(const char *obj, const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller, void *auxiliary, int compositeOffset, bool isCompositeIndirect, const asUINT accessorType)
{
	if( obj == 0 )
	...
	asCDataType dt;
	asCBuilder bld(this, 0);
	int r = bld.ParseDataType(obj, &dt, defaultNamespace, false, true);
	if( r < 0 )
	...
	return RegisterMethodToObjectType(CastToObjectType(dt.GetTypeInfo()), declaration, funcPointer, callConv, caller, auxiliary, compositeOffset, isCompositeIndirect, accessorType);
}

// internal
int asCScriptEngine::RegisterMethodToObjectType(asCObjectType *objectType, const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller, void *auxiliary, int compositeOffset, bool isCompositeIndirect, const asUINT accessorType)
{
#ifdef AS_MAX_PORTABILITY
	if( callConv != asCALL_GENERIC )
	...
	asSSystemFunctionInterface internal;
	int r = DetectCallingConvention(true, funcPointer, callConv, auxiliary, caller, &internal);
	if( r < 0 )
	...
	func->objectType  = objectType;
	func->objectType->AddRefInternal();
	func->exposedType = accessorType;

	asCBuilder bld(this, 0);
	r = bld.ParseFunctionDeclaration(func->objectType, declaration, func, true, nullptr, nullptr);
	if( r < 0 )
	{
	...
	if (func->IsProperty() && (r = bld.ValidatePropertyAccessorFunc(func)) < 0)
	{
		// Set as dummy function before deleting
		func->funcType = asFUNC_DUMMY;
		asDELETE(func, asCScriptFunction);
	...
	if( func->name == "opConv" || func->name == "opImplConv" || func->name == "opCast" || func->name == "opImplCast" )
	{
		...
		int error = 0;
		objectType->FindMethod(func->name.AddressOf(), [&](asCScriptFunction* f)
		{
			if( f->IsSignatureExceptNameEqual(func) )
			{
				func->funcType = asFUNC_DUMMY;
				asDELETE(func,asCScriptFunction);
				error = ConfigError(asALREADY_REGISTERED, "RegisterObjectMethod", objectType->name.AddressOf(), declaration);
			}
		});
		if (error != 0)
			return error;
...
// interface
int asCScriptEngine::RegisterGlobalFunction(const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller, void *auxiliary)
{
#ifdef AS_MAX_PORTABILITY
	if( callConv != asCALL_GENERIC )
	...
	asSSystemFunctionInterface internal;
	int r = DetectCallingConvention(false, funcPointer, callConv, auxiliary, caller, &internal);
	if( r < 0 )
	...
	asCBuilder bld(this, 0);
	r = bld.ParseFunctionDeclaration(0, declaration, func, true, nullptr, nullptr, defaultNamespace);
	if( r < 0 )
	{
		// Set as dummy function before deleting
		func->funcType = asFUNC_DUMMY;
	...
	bool bError = false;
	registeredGlobalFuncTable.FindAll(func->name.AddressOf(), func->nameSpace, [&](asCScriptFunction* f)
	{
		if( f->IsSignatureExceptNameAndReturnTypeEqual(func) )
			bError = true;
	});
	...
	registeredGlobalFuncTable.Add(func);
...
asIScriptFunction *asCScriptEngine::GetGlobalFunctionByIndex(asUINT index) const
{
	...
	return static_cast<asIScriptFunction*>(registeredGlobalFuncs[index]);
}
...
asIScriptFunction *asCScriptEngine::GetGlobalFunctionByDecl(const char *decl) const
{
	...
	while( ns )
	{
		...
		registeredGlobalFuncTable.FindAllUntil(func.name.AddressOf(), ns, [&](asCScriptFunction* funcPtr)
		{
...
void asCScriptEngine::DeprecateGlobalFunctionsByName(const char *name, const char* deprecationMessage)
{
	registeredGlobalFuncTable.FindAll(name, defaultNamespace, [&](asCScriptFunction* ScriptFunction)
	{
		ScriptFunction->traits.SetTrait(asTRAIT_DEPRECATED, true);
#if WITH_EDITOR
		ScriptFunction->deprecationMessage = deprecationMessage;
#endif
	});
}
...
{
	return allRegisteredTypes.FindFirst(type.AddressOf(), type.GetLength(), ns);
}
...
int asCScriptEngine::RegisterDefaultArrayType(const char *type)
{
	asCBuilder bld(this, 0);
	asCDataType dt;
	int r = bld.ParseDataType(type, &dt, defaultNamespace, false, true);
	if( r < 0 ) return r;
...
int asCScriptEngine::RegisterStringFactory(const char *datatype, asIStringFactory *factory)
{
	...
	asCBuilder bld(this, 0);
	asCDataType dt;
	int r = bld.ParseDataType(datatype, &dt, defaultNamespace, true, true);
	if (r < 0)
...
asCModule *asCScriptEngine::GetModule(const char *name, bool create)
{
	...
	asCModule* retModule = scriptModulesByName.FindFirst(name);
...
void asCScriptEngine::RemoveTemplateInstanceType(asCObjectType *t)
{
	...
	asQWORD Bucket = GetTemplateInstanceBucket(t->templateBaseType, t->templateSubTypes);

	t->DestroyInternal();

	auto* BucketArray = templateInstanceBuckets.Find(Bucket);
...
void asCScriptEngine::DiscardTemplateInstance(asCObjectType* TemplateType)
{
	for( asUINT n = 0, Count = TemplateType->templateSubTypes.GetLength(); n < Count; n++ )
	{
		if( auto* SubTypeInfo = TemplateType->templateSubTypes[n].GetTypeInfo() )
		{
			asCModule* SubTypeModule = SubTypeInfo->module;
			if( SubTypeModule != nullptr && SubTypeModule->templateInstances.Contains(TemplateType) )
			{
				SubTypeModule->templateInstances.RemoveSingleSwap(TemplateType);
				TemplateType->ReleaseInternal();
			}
		}
	}

	asQWORD Bucket = GetTemplateInstanceBucket(TemplateType->templateBaseType, TemplateType->templateSubTypes);
	auto* BucketArray = templateInstanceBuckets.Find(Bucket);
	if (BucketArray != nullptr)
		BucketArray->Remove(TemplateType);
}

// internal
asCObjectType *asCScriptEngine::GetTemplateInstanceType(asCObjectType *templateType, asCArray<asCDataType> &subTypes, asCModule*, asCString* outErrorMessage)
{
	asUINT n;

	asQWORD Bucket = GetTemplateInstanceBucket(templateType, subTypes);

	// Is there any template instance type or template specialization already with this subtype?
	asCObjectType* existingInstance = nullptr;

	auto* BucketArray = templateInstanceBuckets.Find(Bucket);
	if (BucketArray != nullptr)
	{
		for (asCObjectType* type : *BucketArray)
		{
			if( type &&
				type->templateBaseType == templateType &&
	...
	{
		// Could be a previously invalid instance
		if (existingInstance->isInvalidGeneratedType)
			return nullptr;
	...
	ot->alignment = templateType->alignment;
	ot->templateBaseType = templateType;
	ot->hasImplicitConstructors = templateType->hasImplicitConstructors;
	ot->module = nullptr;
	...
	bool bUsesAnyScriptTypes = false;
	for( n = 0; n < subTypes.GetLength(); n++ )
	{
		if( auto* TypeInfo = subTypes[n].GetTypeInfo() )
		{
			asCModule* subTypeModule = TypeInfo->module;
			if( subTypeModule != nullptr )
			{
				subTypeModule->templateInstances.Add(ot);

				// The first module we add it to we don't add a reference, because types start with 1 reference
				if (bUsesAnyScriptTypes)
				{
					ot->AddRefInternal();
				}
				else
				{
					ot->module = subTypeModule;
					bUsesAnyScriptTypes = true;
				}
			}
			else
			{
				check((TypeInfo->GetFlags() & asOBJ_SCRIPT_OBJECT) == 0);
			}
		}
	}
	...
	if( templateType->beh.templateCallback )
	{
		...
		if( !deferValidationOfTemplateTypes )
		{
			...
			asCString ErrorString;
			if( !CallGlobalFunctionRetBool(ot, &ErrorString, callback->sysFuncIntf, callback) )
			{
				...
				for( n = 0; n < subTypes.GetLength(); n++ )
				{
					...
					{
						asCModule* subTypeModule = subTypes[n].GetTypeInfo()->module;
						if( subTypeModule != nullptr )
						{
							subTypeModule->templateInstances.RemoveSingleSwap(ot);
							ot->ReleaseInternal();
						}
					}
				}
		...
		{
			unvalidatedTemplateInstances.Add(ot);
		}

		if (!deferCalculatingTemplateSize)
		{
			if ((ot->flags & asOBJ_TEMPLATE_SUBTYPE_DETERMINES_SIZE) != 0)
			{
				ot->CalculateTemplateSize();
			}
		}
	...
	ot->beh.factories = templateType->beh.factories;
	for( n = 0; n < ot->beh.factories.GetLength(); n++ )
		scriptFunctions[ot->beh.factories[n]]->AddRefInternal();
	...
	{
		TArray<asCObjectType*, TInlineAllocator<2>> NewArray;
		NewArray.Add(ot);
		templateInstanceBuckets.Add(Bucket, MoveTemp(NewArray));
	}
	...
	for( n = 0; n < ot->methods.GetLength(); n++ )
	{
		...
		ot->methodTable.Add(func);
	}

	if (ot->beh.destruct != 0)
	{
		int funcId = ot->beh.destruct;
		asCScriptFunction *func = GenerateTemplateFunction(ot, scriptFunctions[funcId]);
	}

	if (ot->beh.copy != 0)
	{
		int funcId = ot->beh.copy;
		asCScriptFunction *func = GenerateTemplateFunction(ot, scriptFunctions[funcId]);
	}
	...
	*(((asWORD*)bc)+1) = (asWORD)func->GetSpaceNeededForArguments() + (func->objectType ? AS_PTR_SIZE : 0);
	...
	if( factory->listPattern )
	{
		...
		while( n )
		{
			...
			{
				asSListPatternDataTypeNode *typeNode = static_cast<asSListPatternDataTypeNode*>(newNode);
...
asCScriptFunction* asCScriptEngine::GenerateTemplateFunction(asCObjectType *ot, asCScriptFunction *func)
{
	...
	func2->returnType = DetermineTypeForTemplate(func->returnType, ot->templateBaseType, ot);
	func2->parameterTypes.SetLength(func->parameterTypes.GetLength());
	for (asUINT p = 0; p < func->parameterTypes.GetLength(); p++)
		func2->parameterTypes[p] = DetermineTypeForTemplate(func->parameterTypes[p], ot->templateBaseType, ot);
	...
	if (ot->beh.destruct == func->id)
	{
		ot->beh.destruct = func2->id;
		func->ReleaseInternal();
	}
	else if (func2->traits.GetTrait(asTRAIT_CONSTRUCTOR))
	{
		for (asUINT n = 0; n < ot->beh.constructors.GetLength(); ++n)
		{
			if (ot->beh.constructors[n] == func->id)
			{
				ot->beh.constructors[n] = func2->id;
				func->ReleaseInternal();
				break;
			}
		}

		if (ot->beh.construct == func->id)
			ot->beh.construct = func2->id;
		if (ot->beh.copyconstruct == func->id)
			ot->beh.copyconstruct = func2->id;
	}
	else
	{
		check(ot->methods.IndexOf(func->id) != -1);

		ot->methodTable.Remove(func);
		ot->methodTable.Add(func2);

		for (asUINT n = 0; n < ot->methods.GetLength(); ++n)
		{
			if (ot->methods[n] == func->id)
			{
				ot->methods[n] = func2->id;
				func->ReleaseInternal();
				break;
			}
		}
	...
	if (ot->beh.copy == func->id)
		ot->beh.copy = func2->id;
...
{
	InScriptFunction ScopeFunction(this, s);
	...
	else if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		if (i->passFirstParamMetaData == asEFirstParamMetaData::ScriptFunction)
		{
...
int asCScriptEngine::CallObjectMethodRetInt(void *obj, int func) const
{
	...
	else if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		if (i->passFirstParamMetaData == asEFirstParamMetaData::ScriptFunction)
		{
			int (*f)(void *, void *) = (int (*)(void *, void *))(i->func);
			return f(obj, s);
		}
		else
		{
...
void *asCScriptEngine::CallObjectMethodRetPtr(void *obj, int param1, asCScriptFunction *func) const
{
	asASSERT( obj != 0 );
	asASSERT( func != 0 );
	InScriptFunction ScopeFunction(this, func);
	...
	if( i->callConv == ICC_GENERIC_METHOD )
	{
		asDWORD paramStack[] = {0, 0, (asDWORD)param1, 0};
		asCGeneric gen(const_cast<asCScriptEngine*>(this), func, obj, (asDWORD*)&paramStack[0]);
		void (*f)(asIScriptGeneric *) = (void (*)(asIScriptGeneric *))(i->func);
		f(&gen);
		return *(void **)gen.GetReturnPointer();
	}
	...
	else if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		if (i->passFirstParamMetaData == asEFirstParamMetaData::ScriptFunction)
		{
			void *(*f)(void *, void *, int) = (void *(*)(void *, void *, int))(i->func);
			return f(obj, func, param1);
		}
		else
		{
			void *(*f)(void *, void *, int) = (void *(*)(void *, void *, int))(i->func);
			return f(obj, func->objectType, param1);
		}
	}
...
{
	InScriptFunction ScopeFunction(this, s);
	if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		if (i->passFirstParamMetaData == asEFirstParamMetaData::ScriptFunction)
		{
			void *(*f)(void*) = (void *(*)(void*))(i->func);
		...
		{
			void *(*f)(void*) = (void *(*)(void*))(i->func);
			return f(s->objectType);
		}
	}
	else if( i->callConv == ICC_CDECL )
	{
		void *(*f)() = (void *(*)())(i->func);
		return f();
	}
...
void asCScriptEngine::CallObjectMethod(void *obj, void *param, asSSystemFunctionInterface *i, asCScriptFunction *s) const
{
	...
	{
		asPWORD paramStack[] = {0, (asPWORD)param};
		asCGeneric gen(const_cast<asCScriptEngine*>(this), s, obj, (asDWORD*)&paramStack[0]);
		void (*f)(asIScriptGeneric *) = (void (*)(asIScriptGeneric *))(i->func);
		f(&gen);
	}
	...
	else if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		if (i->passFirstParamMetaData == asEFirstParamMetaData::ScriptFunction)
		{
			void (*f)(void *, void *, void *) = (void (*)(void *, void *, void *))(i->func);
			return f(obj, s, param);
		}
		else
		{
			void (*f)(void *, void *, void *) = (void (*)(void *, void *, void *))(i->func);
			return f(obj, s->objectType, param);
		}
	}
	else /*if( i->callConv == ICC_CDECL_OBJFIRST */
	{
...
void asCScriptEngine::CallGlobalFunction(void *param1, void *param2, asSSystemFunctionInterface *i, asCScriptFunction *s) const
{
	...
	if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		...
		{
			void (*f)(void *, void *, void *) = (void (*)(void *, void *, void *))(i->func);
			return f(s, param1, param2);
		}
		else
		{
			void (*f)(void *, void *, void *) = (void (*)(void *, void *, void *))(i->func);
			return f(s->objectType, param1, param2);
		}
	}
	else if( i->callConv == ICC_CDECL )
	{
...
bool asCScriptEngine::CallGlobalFunctionRetBool(void *param1, void *param2, asSSystemFunctionInterface *i, asCScriptFunction *s) const
{
	...
	if (i->passFirstParamMetaData != asEFirstParamMetaData::None)
	{
		if (i->passFirstParamMetaData == asEFirstParamMetaData::ScriptFunction)
		{
			bool (*f)(void *, void *, void *) = (bool (*)(void *, void *, void *))(i->func);
			return f(s, param1, param2);
		}
		else
		{
...
void *asCScriptEngine::CallAlloc(const asCObjectType *type) const
{
	...
	return FMemory::Malloc(size, type->alignment);
}

void *asCScriptEngine::AllocScriptObject(const asCObjectType *type) const
{
	if (!(type->flags & asOBJ_SCRIPT_OBJECT))
		return CallAlloc(type);

	if (userAllocScriptObject == nullptr)
		return CallAlloc(type);
	...
	return userAllocScriptObject((asITypeInfo*)type, size);
}

void asCScriptEngine::FinishConstructObject(asIScriptObject* obj, asITypeInfo* type) const
{
	if (userFinishConstructObject != nullptr)
		userFinishConstructObject(obj, type);
}
...
{
	asCTypeInfo *t = static_cast<asCTypeInfo*>(type);
...
int asCScriptEngine::GetTypeIdFromDataType(const asCDataType &dtIn) const
{
	...
	if( dtIn.GetTypeInfo() == 0 )
	{
		...
		case ttFloat32:  return asTYPEID_FLOAT32;
		case ttFloat64: return asTYPEID_FLOAT64;
		default:
...
asCDataType asCScriptEngine::GetDataTypeFromTypeId(int typeId) const
{
	...
	if( typeId <= asTYPEID_FLOAT64 )
	{
		eTokenType type[] = {ttVoid, ttBool, ttInt8, ttInt16, ttInt, ttInt64, ttUInt8, ttUInt16, ttUInt, ttUInt64, ttFloat32, ttFloat64};
		return asCDataType::CreatePrimitive(type[typeId], false);
	}

	// First check if the typeId is an object type
	asCTypeInfo *ot = mapTypeIdToTypeInfo.FindRef(baseId);
	if( ot )
	{
...
{
	mapTypeIdToTypeInfo.Remove(type->GetTypeId());
}
...
asITypeInfo *asCScriptEngine::GetTypeInfoByDecl(const char *decl) const
{
	...
	int r = bld.ParseDataType(decl, &dt, defaultNamespace, false, true);
	if (r < 0)
		return 0;
...
int asCScriptEngine::RefCastObject(void *obj, asITypeInfo *fromType, asITypeInfo *toType, void **newPtr, bool useOnlyImplicitCast)
{
	...
	*newPtr = 0;
	...
	{
		asCFuncdefType *fromFunc = CastToFuncdefType(static_cast<asCTypeInfo*>(fromType));
		asCFuncdefType *toFunc = CastToFuncdefType(static_cast<asCTypeInfo*>(toType));
	...
	asCObjectType *from = static_cast<asCObjectType*>(fromType);
	...
	if( (fromType->GetFlags() & asOBJ_SCRIPT_OBJECT) && (toType->GetFlags() & asOBJ_SCRIPT_OBJECT) )
	{
		...
		if( !useOnlyImplicitCast && (fromType->GetFlags() & asOBJ_REF) )
		{
...
void *asCScriptEngine::CreateScriptObject(const asITypeInfo *type)
{
	...
	asCObjectType *objType = const_cast<asCObjectType*>(static_cast<const asCObjectType *>(type));
	void *ptr = 0;
...
void *asCScriptEngine::CreateUninitializedScriptObject(const asITypeInfo *type)
{
	...
	asCScriptObject *obj = static_cast<asCScriptObject*>(AllocScriptObject(objType));
...
void *asCScriptEngine::CreateScriptObjectCopy(void *origObj, const asITypeInfo *type)
{
	...
	const asCObjectType *ot = static_cast<const asCObjectType*>(type);
...
int asCScriptEngine::AssignScriptObject(void *dstObj, void *srcObj, const asITypeInfo *type)
{
	...
	if( objType->beh.copy )
	{
		...
		if (objType->flags & asOBJ_SCRIPT_OBJECT)
		{
			reinterpret_cast<asCScriptObject*>(dstObj)->PerformCopy(reinterpret_cast<asCScriptObject*>(srcObj), (asCObjectType*)type, (asCObjectType*)type);
		}
		else
		{
...
int asCScriptEngine::RegisterFuncdef(const char *decl)
{
	...
	allRegisteredTypes.Add(fdt); // constructor already set the ref count to 1
	allRegisteredTypesByName.Add(fdt);
...
asCFuncdefType *asCScriptEngine::FindMatchingFuncdef(asCScriptFunction *func, asCModule *module)
{
	...
	if (funcDef == 0)
	{
		...
		fd->parameterOffsets = func->parameterOffsets;
		fd->spaceNeededForArguments = func->spaceNeededForArguments;
		fd->totalSpaceBeforeFunction = func->totalSpaceBeforeFunction;
		...
		if (module)
		{
			...
			module->allLocalTypes.Add(funcDef);
		}
...
int asCScriptEngine::RegisterTypedef(const char *type, const char *decl)
{
	...
	allRegisteredTypes.Add(td);
	allRegisteredTypesByName.Add(td);
...
int asCScriptEngine::RegisterEnumValue(const char *typeName, const char *valueName, int value)
{
	...
	int r;
	asCBuilder bld(this, 0);
	r = bld.ParseDataType(typeName, &dt, defaultNamespace, false, true);
	if( r < 0 )
...
{
	return allRegisteredTypesByName.FindFirst(name);
}

asITypeInfo* asCScriptEngine::GetTypeInfo(const char *name, asSNameSpace* ns) const
{
	asITypeInfo* Type = nullptr;
	allRegisteredTypesByName.FindAllUntil(name, [&](asITypeInfo* CheckType)
	{
		if (((asCTypeInfo*)CheckType)->nameSpace != ns)
			return false;
...
asITypeInfo *asCScriptEngine::GetTypeInfoById(int typeId) const
{
	...
	return mapTypeIdToTypeInfo.FindRef(baseId);
}
...
int asCScriptEngine::GetScriptSectionNameIndex(const char *name)
{
	...
	ScriptSectionName* ExistingName = scriptSectionIndexMap.FindFirst(name);
	if (ExistingName != nullptr)
	{
		return ExistingName->index;
	}

	// Store the script section names for future reference
	ScriptSectionName* SectionName = asNEW(ScriptSectionName)();
	SectionName->name = name;
	SectionName->index = scriptSectionNames.GetLength();

	scriptSectionNames.PushLast(&SectionName->name);
	scriptSectionIndexMap.Add(SectionName);
...
void asCScriptEngine::DestroySubList(asBYTE *&buffer, asSListPatternNode *&node)
{
	...
	while( node )
	{
		...
		else if( node->type == asLPT_TYPE )
		{
			...
			asCDataType dt = static_cast<asSListPatternDataTypeNode*>(node)->dataType;
