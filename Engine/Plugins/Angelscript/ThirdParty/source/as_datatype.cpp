...
asCDataType::asCDataType()
{
	...
	autoConstRef      = false;
	isUnresolvedObject     = false;
	isErrorDataType        = false;
	isFloatExtendedToDouble = false;
	anyImplicitIntegerType = false;
}
...
asCDataType::asCDataType(const asCDataType &dt)
{
	...
	autoConstRef      = dt.autoConstRef;
	isUnresolvedObject     = dt.isUnresolvedObject;
	isErrorDataType     = dt.isErrorDataType;
	isFloatExtendedToDouble = dt.isFloatExtendedToDouble;
	anyImplicitIntegerType = dt.anyImplicitIntegerType;
}
...
bool asCDataType::floatIsFloat64 = true;
asCString asCDataType::Format(asSNameSpace *currNs, bool includeNamespace, bool forDisplay) const
{
	if( IsNullHandle() )
		return "<null handle>";
	if (isErrorDataType)
	...
	bool bRemoveConstForDisplay = false;
	bool bRemoveRefForDisplay = false;
	if (forDisplay)
	{
		if (IsNonPrimitiveValueType() && IsReference() && IsReadOnly())
		{
			bRemoveConstForDisplay = true;
			bRemoveRefForDisplay = true;
		}
	}

	asCString str;

	if( isReadOnly && !bRemoveConstForDisplay )
		str = "const ";
	...
	{
		if (tokenType == ttFloat64)
		{
			if (asCDataType::floatIsFloat64)
			{
				str += "float";
			}
			else
	...
	if( isReference && !bRemoveRefForDisplay )
		str += "&";
...
void asCDataType::MakeConstValueOrConstObject(bool b)
{
	isReadOnly = b;
}
...
bool asCDataType::IsEqualExceptRefAndConst(const asCDataType &dt) const
{
	...
	if (isUnresolvedObject != dt.isUnresolvedObject) return false;
	if( isObjectHandle )
		if( isReadOnly != dt.isReadOnly ) return false;
...
bool asCDataType::IsEqualExceptTypeInfo(const asCDataType &dt) const
{
	// Check base type
	if( tokenType != dt.tokenType ) return false;
	if( (typeInfo == nullptr) != (dt.typeInfo == nullptr) ) return false;
	if( isObjectHandle != dt.isObjectHandle ) return false;
	if (isUnresolvedObject != dt.isUnresolvedObject) return false;
	if( isObjectHandle )
		if( isReadOnly != dt.isReadOnly ) return false;
...
bool asCDataType::IsMathType() const
{
	if( tokenType == ttInt || tokenType == ttInt8 || tokenType == ttInt16 || tokenType == ttInt64 ||
		...
		tokenType == ttFloat32 || tokenType == ttFloat64 )
		return true;
...
bool asCDataType::IsFloat32Type() const
{
	if( tokenType == ttFloat32 )
		return true;

	return false;
}

bool asCDataType::IsFloat64Type() const
{
	if( tokenType == ttFloat64 )
		return true;
...
int asCDataType::GetSizeOfVariableBytes() const
{
	if (typeInfo != nullptr && (typeInfo->flags & asOBJ_REF) != 0)
		return 4*AS_PTR_SIZE;

	return GetSizeInMemoryBytes();
}
...
int  asCDataType::GetAlignment() const
{
	...
	if (typeInfo == NULL)
	{
		if (IsPrimitive())
		{
			if (tokenType == ttBool)
				return 1;
			if (tokenType == ttInt8 || tokenType == ttUInt8)
				return 1;
			if (tokenType == ttInt16 || tokenType == ttUInt16)
				return 2;
			if (tokenType == ttFloat64)
				return alignof(double);
			if (tokenType == ttInt64 || tokenType == ttUInt64)
				return alignof(asINT64);
		}
		return 4;
	}
...
bool asCDataType::IsReferenceType() const
{
	if (typeInfo == 0)
		return false;
	return (typeInfo->GetFlags() & asOBJ_REF) != 0;
}

bool asCDataType::IsNonPrimitiveValueType() const
{
	if (typeInfo == 0)
		return false;
	return (typeInfo->GetFlags() & asOBJ_VALUE) != 0;
}

void asSAccessSpecifier::GetAllowedAccess(bool bAccessedFromDerivedType, asCScriptFunction* accessFromFunction, bool& Read, bool& Write, bool& Edit)
{
	Read = false;
	Write = false;
	Edit = false;

	if (bIsProtected && bAccessedFromDerivedType)
	{
		Read = true;
		Write = true;
		Edit = true;
	...
	if (accessFromFunction != nullptr)
	{
		...
		for (asUINT n = 0, count = permissions.GetLength(); n < count; ++n)
		{
			auto& spec = permissions[n];

			bool bSpecApplies = false;
			if (accessFromType != nullptr)
			{
				if (spec.accessName == accessFromType->name)
					bSpecApplies = true;
				else if (accessFromType->nameSpace->name.GetLength() != 0 && spec.accessName == accessFromType->nameSpace->name)
					bSpecApplies = true;
			}
			else if (spec.accessName == accessFromFunction->name)
			{
				bSpecApplies = true;
			}
			else if (accessFromFunction->nameSpace->name.GetLength() != 0 && spec.accessName == accessFromFunction->nameSpace->name)
			{
				bSpecApplies = true;
			}

			if (spec.bInherited && !bSpecApplies && accessFromType != nullptr)
			{
				auto* superType = accessFromType->derivedFrom;
				while (superType != nullptr)
				{
					if (spec.accessName == superType->name)
					{
						bSpecApplies = true;
						break;
					}
					superType = superType->derivedFrom;
				}

				if (!bSpecApplies)
				{
					superType = (asCObjectType*)accessFromType->shadowType;
					while (superType != nullptr)
					{
						if (spec.accessName == superType->name)
						{
