...
void asCObjectType::DestroyInternal()
{
	...
	for (asUINT n = 0, count = accessSpecifiers.GetLength(); n < count; ++n)
	{
		asDELETE(accessSpecifiers[n], asSAccessSpecifier);
	}
...
void asCObjectType::CopySystemType(asITypeInfo* SystemType)
{
	asCObjectType* Other = (asCObjectType*)SystemType;
	shadowType = Other;

	/*for (int i = 0, Count = Other->properties.GetLength(); i < Count; ++i)
	{
		auto* Prop = Other->properties[i];
		properties.PushLast(Prop);
		propertyTable.Add(Prop);
	}

	for (int i = 0, Count = Other->methods.GetLength(); i < Count; ++i)
	{
		auto* Func = engine->scriptFunctions[Other->methods[i]];
		methods.PushLast(Func->id);
		methodTable.Add(Func);
		Func->AddRefInternal();
	}*/
}
...
{
	asCObjectType *base = (asCObjectType*)shadowType;
	while( base )
	{
		if( base == objType )
			return true;

		base = (asCObjectType*)base->shadowType;
	}
...
// interface
asIScriptFunction *asCObjectType::GetMethodByIndex(asUINT index) const
{
...
// interface
asIScriptFunction *asCObjectType::GetMethodByName(const char *in_name) const
{
	int id = -1;
	FindMethodUntil(in_name, [&](asCScriptFunction* func)
	{
		if (id == -1)
		{
			id = func->id;
...
// interface
asIScriptFunction *asCObjectType::GetMethodByDecl(const char *decl) const
{
...
bool asCObjectType::IsPropertyInherited(asUINT index) const
{
	if( index >= properties.GetLength() )
		return false;

	asCObjectProperty *prop = properties[index];
	int propOffset = prop->byteOffset;

	if (derivedFrom != nullptr && propOffset < derivedFrom->size)
		return true;

	if (shadowType != nullptr && propOffset < basePropertyOffset)
		return true;
...
// internal
asCObjectProperty *asCObjectType::AddPropertyToClass(const asCString &propName, const asCDataType &dt, bool isPrivate, bool isProtected)
{
	...
	asUINT propAlignment = dt.GetAlignment();
	if (propAlignment > (asUINT)alignment)
		alignment = (int)propAlignment;

	properties.PushLast(prop);
	localProperties.PushLast(prop);
	propertyTable.Add(prop);
...
{
	for( asUINT n = 0; n < localProperties.GetLength(); n++ )
	{
		if( localProperties[n] ) 
		{
			if( flags & asOBJ_SCRIPT_OBJECT )
			{
				// Release the config group for script classes that are being destroyed
				asCConfigGroup *group = engine->FindConfigGroupForTypeInfo(localProperties[n]->type.GetTypeInfo());
				if( group != 0 ) group->Release();

				// Release references to objects types
				asCTypeInfo *type = localProperties[n]->type.GetTypeInfo();
				if( type )
			...
			asDELETE(localProperties[n],asCObjectProperty);
		}
	}

	properties.SetLength(0);
	localProperties.SetLength(0);
...
asCObjectProperty* asCObjectType::GetPropertyByOffset(int ByteOffset, asCObjectType** OutObjectType)
{
	auto* checkType = this;
	while (checkType != nullptr)
	{
		int BaseOffset = 0;
		if (checkType->derivedFrom != nullptr)
			BaseOffset = checkType->derivedFrom->size;

		for (int i = 0, Count = checkType->properties.GetLength(); i < Count; ++i)
		{
			if (checkType->properties[i]->byteOffset < BaseOffset)
				continue;

			if (checkType->properties[i]->byteOffset == ByteOffset)
			{
				if (OutObjectType != nullptr)
					*OutObjectType = checkType;
				return checkType->properties[i];
			}
		}

		if (checkType->derivedFrom != nullptr)
			checkType = (asCObjectType*)checkType->derivedFrom;
		else
			checkType = (asCObjectType*)checkType->shadowType;
	}

	return nullptr;
}

asSAccessSpecifier* asCObjectType::GetAccessSpecifier(const char* specName)
{
	for (asUINT n = 0, count = accessSpecifiers.GetLength(); n < count; ++n)
	{
		if (accessSpecifiers[n]->name == specName)
			return accessSpecifiers[n];
	}
	return nullptr;
}
