...
asCByteCode::asCByteCode(asCBuilder* builder)
{
	first = 0;
	last  = 0;
	largestStackUsed = -1;
...
bool asCByteCode::RemoveUnusedValue(asCByteInstruction *curr, asCByteInstruction **next)
{
	...
	if( curr->op == asBC_SetV8 && curr->next )
	{
		// The constant value is immediately moved to another variable and then not used again
		if( curr->next->op == asBC_CpyVtoV8 &&
			curr->wArg[0] == curr->next->wArg[1] &&
			IsTemporary(curr->wArg[0]) &&
			!IsTempVarRead(curr->next, curr->wArg[0]) )
		{
	...
	if( (asBCInfo[curr->op].type == asBCTYPE_wW_rW_rW_ARG ||
		 asBCInfo[curr->op].type == asBCTYPE_wW_rW_DW_ARG) &&
		curr->next && (curr->next->op == asBC_CpyVtoV4 || curr->next->op == asBC_CpyVtoV8) &&
		curr->wArg[0] == curr->next->wArg[1] &&
		IsTemporary(curr->wArg[0]) &&
		!IsTempVarRead(curr->next, curr->wArg[0]) )
	{
	...
	if( (curr->op == asBC_CpyRtoV4 || curr->op == asBC_CpyRtoV8) && curr->next && (curr->next->op == asBC_CpyVtoR4 || curr->next->op == asBC_CpyVtoR8 || curr->next->op == asBC_CpyVtoR1) &&
		curr->wArg[0] == curr->next->wArg[0] &&
		IsTemporary(curr->wArg[0]) &&
		!IsTempVarRead(curr->next, curr->wArg[0]) )
	{
...
void asCByteCode::OptimizeLocally(const asCArray<int> &tempVariableOffsets)
{
	...
	while( instr )
	{
		...
		{
			// PopRPtr, PshRPtr -> Nothing
			if( instr && instr->op == asBC_PshRPtr )
			{
				DeleteInstruction(curr);
				instr = DeleteInstruction(instr);
			}
...
{
	void *ptr = builder->MemStack.Alloc(sizeof(asCByteInstruction), alignof(asCByteInstruction));
...
void asCByteCode::Call(asEBCInstr instr, int funcID, int pop)
{
	...
	if (instr == asBC_CALLSYS || instr == asBC_Thiscall1)
	{
		asASSERT(asBCInfo[instr].type == asBCTYPE_PTR_ARG);
		asCScriptFunction *f = engine->scriptFunctions[funcID];
		*ARG_PTR(last->arg) = (asPWORD)f;
	}
	else
	{
...
void asCByteCode::Alloc(asEBCInstr instr, void *objID, int funcID, int pop, bool valueType)
{
	if( AddInstruction() < 0 )
		return;
	...
	if (valueType)
	{
		if ((((asITypeInfo*)objID)->GetFlags() & asOBJ_SCRIPT_OBJECT) == 0)
			valueType = false;
	}
...
void asCByteCode::JmpP(int var, asDWORD max)
{
	...
	asASSERT(asBCInfo[asBC_JMPP].type == asBCTYPE_rW_DW_ARG);
...
void asCByteCode::PostProcess()
{
	...
	for( asUINT p = 0; p < paths.GetLength(); ++p )
	{
		...
		while( instr )
		{
			...
			if (instr->op == asBC_ALLOC)
			{
				// Alloc instructions do an internal push before popping so should
				// make sure the stack size is large enough
				if (stackSize + 2 > largestStackUsed)
					largestStackUsed = stackSize + 2;
			}
...
int asCByteCode::InstrW_W(asEBCInstr bc, int a, int b)
{
	asASSERT(asBCInfo[bc].type == asBCTYPE_wW_rW_ARG ||
	         ...
	         asBCInfo[bc].type == asBCTYPE_W_rW_ARG);
	asASSERT(asBCInfo[bc].stackInc == 0);
