...
bool asCString::Equals(const char* str) const
{
	if (!str)
		return false;

	const char* A = AddressOf();
	const char* B = str;
	while (*A != '\0')
	{
		if (*A != *B)
...
bool asCString::Equals(const char* str, size_t in_length) const
{
	if (length != in_length)
		return false;
	if (length == 0)
		return true;
	return memcmp(AddressOf(), str, length) == 0;
}

bool asCString::Equals(const asCString& str) const
{
	if (length != str.GetLength())
		return false;
	if (length == 0)
		return true;
	return memcmp(AddressOf(), str.AddressOf(), length) == 0;
}

bool asCString::Equals_CaseInsensitive(const asCString& str) const
{
	if (length != str.GetLength())
		return false;
	if (length == 0)
	...
	const char* A = AddressOf();
	const char* B = str.AddressOf();
	while (*A != '\0')
	{
		if (RemoveCase(*A) != RemoveCase(*B))
			return false;
		++A;
		++B;
	}
...
bool asCString::StartsWith(const char* str) const
{
	unsigned int len = strlen(str);
	return len > length ? false : memcmp(AddressOf(), str, len) == 0;
}
bool asCString::EndsWith(const char* str) const
{
	unsigned int len = strlen(str);
	return len > length ? false : memcmp(AddressOf() + (length - len), str, len) == 0;
}

int32 asCString::LevenshteinDistance(const asCString& other)
{
	const int32 LenA = GetLength();
	const int32 LenB = other.GetLength();
	...
	auto DataA = AddressOf();
	auto DataB = other.AddressOf();
	...
	for (int32 IndexA = 0; IndexA < LenA; ++IndexA)
	{
		...
		for (int32 IndexB = 0; IndexB < LenB; ++IndexB)
		{
			...
			if (asCString::RemoveCase(DataA[IndexA]) != asCString::RemoveCase(DataB[IndexB]))
			{
