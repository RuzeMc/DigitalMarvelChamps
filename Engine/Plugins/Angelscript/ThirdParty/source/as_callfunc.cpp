...
int PrepareSystemFunction(asCScriptFunction *func, asSSystemFunctionInterface *internal, asCScriptEngine *engine)
{
	...
	else if( func->returnType.GetSizeInMemoryDWords() == 2 )
	{
		...
		internal->hostReturnFloat    = func->returnType.IsEqualExceptConst(asCDataType::CreatePrimitive(ttFloat64, true));
	}
	else if( func->returnType.GetSizeInMemoryDWords() == 1 )
	{
		internal->hostReturnInMemory = false;
		internal->hostReturnSize     = 1;
		internal->hostReturnFloat    = func->returnType.IsEqualExceptConst(asCDataType::CreatePrimitive(ttFloat32, true));
	}
	else
	{
		internal->hostReturnInMemory = false;
	...
	for( n = 0; n < func->parameterTypes.GetLength(); n++ )
	{
		...
		{
			int alignedSize = func->parameterTypes[n].GetSizeInMemoryBytes();
			internal->maxNativeStackSize += (alignedSize + 7) & (~7);
...
//asQWORD CallSystemFunctionNative(asCScriptEngine *engine, asCScriptFunction *descr, void *obj, asDWORD *args, void *retPointer, asQWORD &retQW2, void *secondObj);
...
int CallSystemFunction(int id, asCContext *context)
{
	...
	retQW = CallSystemFunctionNative(context->m_engine, descr, obj, args, sysFunc->hostReturnInMemory ? retPointer : 0, retQW2, secondObj);
#else
