...
enum asEEngineProp
{
	...
	asEP_AUTOMATIC_IMPORTS                  = 29,
	asEP_TYPECHECK_SWITCH_ENUMS             = 30,
	asEP_ALLOW_DOUBLE_TYPE             = 31,
	asEP_FLOAT_IS_FLOAT64             = 32,
	asEP_WARN_ON_FLOAT_CONSTANTS_FOR_DOUBLES  = 33,
	asEP_WARN_INTEGER_DIVISION  = 34,
...
enum asEObjTypeFlags
{
	...
	asOBJ_TEMPLATE_SUBTYPE_COVARIANT  = (1<<16),
	asOBJ_TEMPLATE_SUBTYPE_DETERMINES_SIZE = (1<<17),
	asOBJ_NOCOUNT                    = (1<<18),
	asOBJ_DISALLOW_INSTANTIATION     = (1<<19),
	asOBJ_IMPLICIT_HANDLE            = (1<<20),
	asOBJ_BASICMATHTYPE              = (1<<21),
	asOBJ_MASK_VALID_FLAGS           = 0x3FFFFF,
	// Internal flags
	asOBJ_SCRIPT_OBJECT              = (1<<22),
	asOBJ_SHARED                     = (1<<23),
	asOBJ_NOINHERIT                  = (1<<24),
	asOBJ_FUNCDEF                    = (1<<25),
	asOBJ_LIST_PATTERN               = (1<<26),
	asOBJ_ENUM                       = (1<<27),
	asOBJ_TEMPLATE_SUBTYPE           = (1<<28),
	asOBJ_TYPEDEF                    = (1<<29),
	asOBJ_ABSTRACT                   = (1<<30),
	asOBJ_EDITOR_ONLY                = (1<<31)
};
...
typedef void *(*asALLOCSCRIPTOBJECTFUNC_t)(asITypeInfo*, size_t);
typedef void (*asFINISHCONSTRUCTOBJECTFUNC_t)(asIScriptObject*, asITypeInfo*);
...
typedef void (*asRESOLVEOBJECTPTRFUNC_t)(void**);
...
extern "C"
{
	...
	AS_API int   asSetAllocScriptObjectFunction(asALLOCSCRIPTOBJECTFUNC_t allocFunc, asFINISHCONSTRUCTOBJECTFUNC_t finishFunc);
	AS_API int   asSetResolveObjectPtrFunction(asRESOLVEOBJECTPTRFUNC_t resolveFunc);
...
struct asFunctionCaller
{
	...
	union
	{
		MethodCallerPtr MethodCaller;
		FunctionCallerPtr FunctionCaller;
	};
	int type;

	asFunctionCaller() : FunctionCaller(nullptr), type(0) {}
	asFunctionCaller(nullptr_t) : FunctionCaller(nullptr), type(0) {}
	asFunctionCaller(FunctionCallerPtr InFunctionCaller) : FunctionCaller(InFunctionCaller), type(1) {}
	asFunctionCaller(MethodCallerPtr InMethodCaller) : MethodCaller(InMethodCaller), type(2) {}

	bool IsBound() const { return MethodCaller != nullptr; }
};
...
public:
	...
	// Global functions
	virtual int                RegisterGlobalFunction(const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller = nullptr, void *auxiliary = 0) = 0;
	...
	virtual int            RegisterObjectProperty(const char *obj, const char *declaration, int byteOffset, int compositeOffset = 0, bool isCompositeIndirect = false, const asUINT accessorType = 255, bool isProtected = false) = 0;
	virtual int            RegisterObjectMethod(const char *obj, const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller = nullptr, void *auxiliary = 0, int compositeOffset = 0, bool isCompositeIndirect = false, const asUINT accessorType = 255) = 0;
	virtual int            RegisterObjectBehaviour(const char *obj, asEBehaviours behaviour, const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, asFunctionCaller caller = nullptr, void *auxiliary = 0, int compositeOffset = 0, bool isCompositeIndirect = false) = 0;
...
struct asPreClassData
{
	size_t PropertyOffset = 0;
	asITypeInfo* ShadowType = nullptr;
	void* InitialUserData = nullptr;
};
...
public:
	...
	// Compilation
	virtual void        AddPreClassData(const char* name, asPreClassData Data) = 0;
	...
	virtual void ImportModule(asIScriptModule* module) = 0;
...
public:
	...
	/* Internal*/
	asIScriptObject &operator=(const asIScriptObject &other);

	// Used for properties
	void *AllocateUninitializedObject(class asCObjectType *objType, class asCScriptEngine *engine);
	//void FreeObject(void *ptr, asCObjectType *objType, asCScriptEngine *engine);
	void CopyObject(void *src, void *dst, class asCObjectType *objType, class asCScriptEngine *engine);
	void CopyHandle(asPWORD *src, asPWORD *dst, class asCObjectType *objType, class asCScriptEngine *engine);

	void CallDestructor(class asCObjectType* objType);
};
...
public:
	...
	virtual bool             ShadowsFrom(const asITypeInfo *objType) const = 0;
	...
	virtual asIScriptFunction *GetMethodByIndex(asUINT index) const = 0;
	virtual asIScriptFunction *GetMethodByName(const char *name) const = 0;
	virtual asIScriptFunction *GetMethodByDecl(const char *decl) const = 0;
	...
	virtual bool        IsPropertyInherited(asUINT index) const = 0;
...
public:
	...
	virtual const char      *GetName() const = 0;
	virtual const char      *GetNamespace() const = 0;
	virtual const char      *GetDeclaration(bool includeObjectName = true, bool includeNamespace = false, bool includeParamNames = false, bool includeHiddenParam = false) const = 0;
...
typedef void (*asJITFunction)(struct FScriptExecution& Execution, asDWORD* l_fp, asQWORD* l_outValue);
typedef void (*asJITFunction_ParmsEntry)(struct FScriptExecution& Execution, void* Object, void* Parms);
typedef void (*asJITFunction_Raw)();
...
enum asEBCInstr
{
	...
	asBC_POWu64			= 199,
	asBC_Thiscall1		= 200,
	asBC_FinConstruct	= 201,
	asBC_DestructScript	= 202,
	asBC_CopyScript 	= 203,
	asBC_ResolveObjectPtr = 204,
	asBC_FreeNullV8 = 205,
	asBC_TrackRef = 206,
	asBC_UntrackRef = 207,
	asBC_ValidateRef = 208,
	asBC_CpyVtoR1		= 209,
	asBC_SaveReturnValue		= 210,
	asBC_CmpPtrNull		= 211,
	asBC_ThrowException		= 212,
	asBC_PshV1		= 213,

	asBC_MAXBYTECODE	= 213,
...
enum asEBCType
{
	...
	asBCTYPE_W_rW_ARG     = 21,
};

// Instruction type sizes
const int asBCTypeSize[22] =
{
	0, // asBCTYPE_INFO
	1, // asBCTYPE_NO_ARG
	1, // asBCTYPE_W_ARG
...
const asSBCInfo asBCInfo[256] =
{
	...
	asBCINFO(GETOBJ,	W_rW_ARG,		0),
	asBCINFO(REFCPY,	NO_ARG,		-AS_PTR_SIZE),
	asBCINFO(CHKREF,	NO_ARG,			0),
	asBCINFO(GETOBJREF,	W_rW_ARG,		0),
	asBCINFO(GETREF,	W_rW_ARG,		0),
	...
	asBCINFO(iTOf,		wW_rW_ARG,			0),
	asBCINFO(fTOi,		wW_rW_ARG,			0),
	asBCINFO(uTOf,		wW_rW_ARG,			0),
	asBCINFO(fTOu,		wW_rW_ARG,			0),
	asBCINFO(sbTOi,		wW_rW_ARG,			0),
	asBCINFO(swTOi,		wW_rW_ARG,			0),
	asBCINFO(ubTOi,		wW_rW_ARG,			0),
	asBCINFO(uwTOi,		wW_rW_ARG,			0),
	...
	asBCINFO(iTOb,		wW_rW_ARG,			0),
	asBCINFO(iTOw,		wW_rW_ARG,			0),
	...
	asBCINFO(dTOi64,	wW_rW_ARG,			0),
	asBCINFO(fTOu64,	wW_rW_ARG,		0),
	asBCINFO(dTOu64,	wW_rW_ARG,			0),
	asBCINFO(i64TOf,	wW_rW_ARG,		0),
	asBCINFO(u64TOf,	wW_rW_ARG,		0),
	asBCINFO(i64TOd,	wW_rW_ARG,			0),
	asBCINFO(u64TOd,	wW_rW_ARG,			0),
	...
	asBCINFO(RefCpyV,	wW_ARG,		-AS_PTR_SIZE),
	...
	asBCINFO(Thiscall1, PTR_ARG,			-AS_PTR_SIZE-1),
	asBCINFO(FinConstruct, PTR_ARG,		-AS_PTR_SIZE),
	asBCINFO(DestructScript, rW_PTR_ARG,	0),
	asBCINFO(CopyScript,	PTR_ARG,		-AS_PTR_SIZE),
	asBCINFO(ResolveObjectPtr,	NO_ARG,		0),
	asBCINFO(FreeNullV8,		wW_ARG,		0),
	asBCINFO(TrackRef,		rW_ARG,		0),
	asBCINFO(UntrackRef,		rW_ARG,		0),
	asBCINFO(ValidateRef,		rW_ARG,		0),
	asBCINFO(CpyVtoR1,	rW_ARG,			0),
	asBCINFO(SaveReturnValue,		NO_ARG,			0),
	asBCINFO(CmpPtrNull,	rW_ARG,		0),
	asBCINFO(ThrowException,	W_ARG,		0),
	asBCINFO(PshV1,		rW_ARG,			1),
